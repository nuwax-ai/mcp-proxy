...
统一部署 CLI（npm 包 `@nuwax-ai/deploy-installer`，命令 `deploy-installer`）。
...
- **推荐**：`npm i -g @nuwax-ai/deploy-installer`（`@latest`）→ `deploy-installer voice-cli|document-parser …`；尝鲜用 `@beta`
