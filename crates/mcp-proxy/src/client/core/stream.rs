//! Streamable HTTP protocol adapter for the shared remote convert runtime.
...
use mcp_streamable_proxy::{FallbackMetadata, ServiceExt, stdio as stream_stdio};
...
#[async_trait]
impl RemoteProtocolAdapter for StreamAdapter {
    type Connection = StreamClientConnection;
    type Handler = StreamProxyHandler;
    ...
    fn parse_import(&self, json: LoadedFallbackJson) -> Result<Self::ImportedFallback> {
        ...
        FallbackMetadata::from_json(&json.initialize.json, &json.tools.json).with_context(|| {
            format!("failed to parse Streamable HTTP fallback metadata ({sources})")
        })
    }

    async fn connect_initial(&self, config: McpClientConfig) -> Result<Self::Connection> {
        StreamClientConnection::connect(config).await
    }
    ...
    async fn connect_reconnect(
        ...
        StreamClientConnection::connect_with_peers(config, handler.upstream_peers()).await
    }
    ...
    async fn build_handler(
            ...
            match connection.as_ref() {
                ...
                .await
                {
                    ...
                    Err(error) => {
                        tracing::warn!(
                            %error,
                            "real Streamable HTTP tools discovery failed; retaining imported fallback"
                        );
                        None
                    }
                },
        ...
        let initial_info = live_snapshot
            ...
            .or_else(|| {
                connection
                    .as_ref()
                    .and_then(|connection| connection.peer_info().map(|info| (*info).clone()))
            })
            .or_else(|| {
                imported
                    .as_ref()
            ...
            .context("Streamable HTTP fallback is missing initialize metadata")?;
        ...
        let handler = Arc::new(StreamProxyHandler::new_disconnected_with_fallback(
            "cli".to_string(),
            filter,
            initial_info,
            initial_tools,
        ...
        if let Some(connection) = connection {
            ...
            tracing::warn!(
                "starting Streamable HTTP stdio with imported discovery fallback while disconnected"
            );
        }
    ...
    async fn install_reconnected(
        ...
        .await
        {
            ...
            Err(error) => {
                tracing::warn!(
                    %error,
                    "Streamable HTTP reconnect tools discovery failed; retaining previous cache"
                );
                None
            }
        };
    ...
    async fn serve_stdio(&self, handler: Arc<Self::Handler>) -> Result<()> {
        let server = (*handler)
            ...
            .serve(stream_stdio())
            .await
            .context("failed to start Streamable HTTP stdio server")?;
        server
            .waiting()
            .await
            .context("Streamable HTTP stdio server terminated with an error")
            .map(|_| ())
    }
}

pub(super) async fn run_stream_mode(
    config: McpClientConfig,
    discovery_mode: DiscoveryMode,
    tool_filter: ToolFilter,
    options: RuntimeOptions,
) -> Result<()> {
    run_remote_mode(StreamAdapter, config, discovery_mode, tool_filter, options).await
}
