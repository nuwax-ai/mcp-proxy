...
fn path_writable(dir: &Path) -> bool {
    ...
    let probe = dir.join(".systemd_installer_write_probe");
...
fn unit_is_active(name: &str) -> bool {
...
pub struct PrecheckOptions {
    /// When true, missing/unusable sudo is a hard failure (skip for `--dry-run`).
...
pub fn precheck(spec: &ServiceSpec, opts: &PrecheckOptions) -> Result<PrecheckReport> {
    ...
    {
        let self_active = unit_is_active(&spec.name);
    ...
    // 7. existing unit
    let unit = spec.unit_path();
    if unit.exists() {
        report.push(
            "existing_unit",
            CheckSeverity::Warn,
            format!("{} exists and will be overwritten", unit.display()),
        );
    } else {
