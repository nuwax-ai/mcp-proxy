...
pub fn get_or_init_model(
    ...
    let embedding = TextEmbedding::try_new(options)
        .with_context(|| format!("无法初始化模型: {:?}", model))?;
...
impl ModelInfo {
    pub fn from_embedding_model(model: &EmbeddingModel) -> Self {
        let (variant, code, dim) = match model {
            ...
            EmbeddingModel::AllMiniLML6V2 => ("AllMiniLML6V2", "sentence-transformers/all-MiniLM-L6-v2", 384),
            EmbeddingModel::AllMiniLML12V2 => ("AllMiniLML12V2", "sentence-transformers/all-MiniLM-L12-v2", 384),
            _ => ("Unknown", "unknown", 0),
        };
