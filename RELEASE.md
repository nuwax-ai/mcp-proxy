...
> **两条发布路线**：本仓库的 tag 分两族——`v*` 走本文档的 cargo-dist（GitHub Releases 二进制）；`deploy-v*` 走 npm 包 `@nuwax-ai/deploy-installer`（三平台部署器，release workflow 会跳过）。后者见 [crates/deploy-installer/doc/MAINTAINER.md](crates/deploy-installer/doc/MAINTAINER.md)。
