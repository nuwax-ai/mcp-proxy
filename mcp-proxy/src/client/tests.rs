...
mod reconnection_tests {
    ...
    fn get_test_mcp_server_path() -> String {
        // 使用 CARGO_TARGET_DIR 或默认的 target 目录
        let manifest_dir = env!("CARGO_MANIFEST_DIR");
        // 测试时二进制在 target/debug 目录
    ...
    /// 启动 proxy 服务器
    async fn spawn_proxy_server(port: u16) -> anyhow::Result<Child> {
    ...
    /// 等待服务器就绪（TCP 轮询）
    async fn wait_for_server_ready(addr: &str, max_retries: u32) -> anyhow::Result<()> {
    ...
    async fn wait_for_stderr_pattern(
    ...
    /// 发送 JSON-RPC 请求并获取响应
    async fn send_jsonrpc_and_receive(
    ...
    async fn test_reconnection_normal_connection() {
        ...
        let mut proxy = spawn_proxy_server(TEST_PORT).await.expect("启动 proxy 失败");
    ...
    async fn test_reconnection_on_server_restart() {
        ...
        // 1. 启动 proxy 服务器
        println!("🚀 启动 proxy 服务器...");
        let mut proxy = spawn_proxy_server(TEST_PORT + 1)
            .await
            .expect("启动 proxy 失败");
    ...
    async fn test_reconnection_exponential_backoff() {
        ...
        let url = format!("http://127.0.0.1:{}", TEST_PORT + 2);
    ...
    async fn test_request_returns_error_when_connection_closed() {
        ...
        // 1. 启动 proxy 服务器
        println!("🚀 启动 proxy 服务器...");
        let mut proxy = spawn_proxy_server(TEST_PORT + 3).await.expect("启动 proxy 失败");

        let addr = format!("127.0.0.1:{}", TEST_PORT + 3);
        wait_for_server_ready(&addr, 20)
            .await
            .expect("服务器启动超时");
