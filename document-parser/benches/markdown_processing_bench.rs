...
fn markdown_processing_benchmark(c: &mut Criterion) {
        ...
        group.bench_with_input(
            ...
            |b, content| {
                b.to_async(&rt).iter(|| async {
                    processor
                        .parse_markdown_with_toc(black_box(content))
                        .await
                        .unwrap()
                });
            },
    ...
    // 基准测试：流式Markdown解析
    for (name, content) in [("medium", &medium_doc), ("large", &large_doc)] {
        group.bench_with_input(
    ...
    group.bench_with_input(
        ...
        |b, content| {
            b.to_async(&rt).iter(|| async {
                processor
                    .extract_table_of_contents(black_box(content))
                    .await
                    .unwrap()
            });
        },
    ...
    // 基准测试：内容搜索
    let doc_structure = rt
        .block_on(processor.parse_markdown_with_toc(&medium_doc))
        .unwrap();
    group.bench_function("search_content", |b| {
        b.to_async(&rt).iter(|| async {
            processor
                .search_content(black_box(&doc_structure), black_box("测试"))
                .await
        });
    });
    ...
    group.bench_function("batch_process", |b| {
        b.to_async(&rt).iter(|| async {
            processor
                .batch_process_documents(black_box(batch_docs.clone()))
                .await
                .unwrap()
        });
    });
