...
use crate::models::{DocumentFormat, SourceType, TaskStatus};
...
impl RequestValidator {
    ...
    pub fn validate_sort_params(
        ...
        let allowed_sort_fields = HashSet::from([
            "created_at", "updated_at", "progress", "file_size"
        ]);
    ...
    pub fn validate_document_format(format: &DocumentFormat) -> Result<(), AppError> {
        // 这里可以添加特定格式的验证逻辑
        match format {
            DocumentFormat::PDF | DocumentFormat::Word | DocumentFormat::Excel | 
            DocumentFormat::PowerPoint | DocumentFormat::Image | DocumentFormat::Audio |
            DocumentFormat::HTML | DocumentFormat::Text | DocumentFormat::Txt | 
            DocumentFormat::Md | DocumentFormat::Other(_) => Ok(()),
        }
    }
    
    /// 验证源类型
    pub fn validate_source_type(source_type: &SourceType) -> Result<(), AppError> {
        match source_type {
            SourceType::Upload | SourceType::Url | SourceType::Oss => Ok(()),
