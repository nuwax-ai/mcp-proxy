...
use document_parser::{
    AppConfig, AppState, AppError, APP_NAME, APP_VERSION,
    routes::create_routes,
    config::init_global_config,
    utils::environment_manager::{EnvironmentManager, EnvironmentStatus, InstallStage, DirectoryValidationResult, CleanupRisk}
};
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Parse { input, output, parser }) => {
            return handle_parse_command(&app_config, &environment_manager, input, output, parser).await;
        }
...
async fn handle_uv_init_command(_environment_manager: &EnvironmentManager) -> Result<()> {
    ...
    let _validation_result = match local_env_manager.check_current_directory_readiness().await {
        Ok(result) => {
            if result.is_valid {
                ...
                // 尝试自动修复可修复的问题
                let auto_fixable: Vec<_> = result.issues.iter()
                    ...
                    for cleanup_option in &result.cleanup_options {
                        if cleanup_option.risk_level == CleanupRisk::Low ||
                           cleanup_option.risk_level == CleanupRisk::Medium {
                            match local_env_manager.execute_cleanup_option(cleanup_option.option_type.clone()).await {
    ...
    let env_status = match local_env_manager.check_environment().await {
        Ok(status) => {
            println!("   环境检查完成:");
            println!("   Python:     {}", if status.python_available { "✅ 可用" } else { "❌ 不可用" });
            println!("   uv工具:     {}", if status.uv_available { "✅ 可用" } else { "❌ 不可用" });
            println!("   虚拟环境:   {}", if status.virtual_env_active { "✅ 激活" } else { "❌ 未激活" });
            println!("   MinerU:     {}", if status.mineru_available { "✅ 可用" } else { "❌ 不可用" });
            println!("   MarkItDown: {}", if status.markitdown_available { "✅ 可用" } else { "❌ 不可用" });
            println!();
            status
        }
        Err(e) => {
        ...
        while let Some(progress) = progress_rx.recv().await {
            ...
            if progress.package != last_package || (progress.progress - last_progress).abs() > 10.0 {
                let stage_icon = match progress.stage {
                    ...
                    InstallStage::Retrying { attempt, max_attempts } => {
                        println!("   🔄 重试 {}/{}: {}", attempt, max_attempts, progress.message);
                        continue;
                    }
                };
        ...
        match env_manager_with_progress.auto_fix_venv_path_issues().await {
            Ok(fixed) => {
                if !fixed.is_empty() {
                    ...
                    // 显示详细的恢复建议
                    let suggestions = env_manager_with_progress.get_venv_recovery_suggestions().await;
    ...
    match install_result {
        ...
        Err(e) => {
            ...
            println!("💡 详细故障排除建议:");
            match &e {
                AppError::VirtualEnvironmentPath(_) | AppError::Permission(_) | AppError::Path(_) => {
                    for suggestion in e.get_path_recovery_suggestions() {
                        println!("   • {}", suggestion);
                    }
                }
...
fn print_success_message(current_dir: &std::path::Path) {
    ...
    println!("💡 提示:");
    println!("   • 虚拟环境位置: ./venv/");
    println!("   • Python可执行文件: ./venv/bin/python (Linux/macOS) 或 .\\venv\\Scripts\\python.exe (Windows)");
...
async fn handle_troubleshoot_command(environment_manager: &EnvironmentManager) -> Result<()> {
    ...
    println!("   虚拟环境: ./venv/");
    println!("   操作系统: {}", if cfg!(windows) { "Windows" } else if cfg!(target_os = "macos") { "macOS" } else { "Linux" });
    println!();
    ...
    println!("❓ 问题: 虚拟环境激活失败");
    println!("🔍 诊断步骤:");
    println!("   1. 检查虚拟环境是否存在: ls ./venv/bin/ (Linux/macOS) 或 dir .\\venv\\Scripts\\ (Windows)");
    println!("   2. 检查激活脚本权限");
    println!();
    println!("💡 解决方案:");
    if cfg!(windows) {
        ...
        println!("   • 如果PowerShell执行策略限制，运行: Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser");
    } else {
    ...
    if cfg!(target_os = "macos") {
        ...
        println!("     - Ubuntu/Debian: 参考 https://docs.astral.sh/uv/getting-started/installation/");
    } else if cfg!(windows) {
            ...
            match environment_manager.check_environment().await {
                Ok(status) => {
                    if status.is_ready() {
                        ...
                        if !auto_fixable.is_empty() {
                            println!("💡 {} 个问题可以自动修复，运行 'document-parser uv-init' 进行修复", auto_fixable.len());
                        }
                        ...
                        Err(anyhow::anyhow!("环境未就绪，健康评分: {}/100", status.health_score()))
                    }
                }
                Err(e) => {
                    println!("❌ 环境状态检查失败: {}", e);
...
async fn handle_parse_command(
    ...
    // 检查环境
    let env_status = environment_manager.check_environment().await
        .map_err(|e| anyhow::anyhow!("环境检查失败: {}", e))?;
    
    match parser.as_str() {
        "mineru" => {
            if !env_status.mineru_available {
                return Err(anyhow::anyhow!("MinerU未安装，请先运行 'document-parser install'"));
            }
        }
        "markitdown" => {
            if !env_status.markitdown_available {
                return Err(anyhow::anyhow!("MarkItDown未安装，请先运行 'document-parser install'"));
            }
        }
        _ => {
            return Err(anyhow::anyhow!("不支持的解析器: {}，支持的解析器: mineru, markitdown", parser));
        }
    }
    
    // 创建应用状态（用于解析器）
    let _state = AppState::new(app_config.clone()).await
        .map_err(|e| anyhow::anyhow!("无法创建应用状态: {}", e))?;
