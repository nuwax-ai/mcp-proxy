...
use deployment_health::{HealthCheckManager, HealthChecker, DatabaseHealthChecker};
use resource_cleanup::{ResourceCleaner, DatabaseConnectionCleaner};
...
impl ProductionManager {
    ...
    pub async fn new(config: AppConfig) -> Result<Self, AppError> {
        ...
        let config_validator = Arc::new(ConfigValidator::new(config_validation::ValidationRules::default()));
        let logger = Arc::new(ProductionLogger::new(production_logging::LoggingConfig::default()));
        let monitoring = Arc::new(MonitoringIntegration::new(monitoring_integration::MonitoringConfig::default()));
        ...
        let resource_cleaner: Arc<dyn ResourceCleaner + Send + Sync> = Arc::new(
            DatabaseConnectionCleaner::new(
                "production_db_cleaner".to_string(),
                10,
            )
        );
    ...
    pub async fn validate_configuration(&self) -> Result<(), AppError> {
        ...
        let result = validator.validate_config(&self.config)
    ...
    /// 初始化健康检查
    pub async fn initialize_health_checks(&self, _app_state: Arc<AppState>) -> Result<(), AppError> {
    ...
    pub async fn setup_graceful_shutdown(&self, app_state: Arc<AppState>) -> Result<(), AppError> {
        self.shutdown_manager.setup(app_state, self.resource_cleaner.clone()).await
    }
    ...
    pub async fn get_runtime_stats(&self) -> Result<RuntimeStats, AppError> {
        ...
        let uptime = deployment_info.started_at
            .elapsed()
    ...
    pub async fn run_production_checks(&self) -> Result<ProductionCheckResult, AppError> {
        ...
        match self.get_health_status().await {
            Ok(health) => {
                let status = if health.overall_status == deployment_health::HealthCheckStatus::Healthy {
                    CheckStatus::Passed
                } else {
                    CheckStatus::Warning
                };
