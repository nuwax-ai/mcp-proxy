...
impl EnvironmentStatus {
    ...
    pub fn is_virtual_env_properly_configured(&self) -> bool {
        ...
        if let Some(ref venv_path) = self.virtual_env_path {
            // 检查路径是否以当前目录的venv结尾
            venv_path.ends_with("venv") || venv_path.contains("/venv") || venv_path.contains("\\venv")
        } else {
            false
        }
    }
    ...
    pub fn generate_diagnostic_report(&self) -> DiagnosticReport {
        let mut report = DiagnosticReport {
            overall_status: if self.is_ready() { "Ready" } else { "Not Ready" }.to_string(),
        ...
        let python_component = ComponentDiagnostic {
            ...
            status: if self.python_available { "Available" } else { "Missing" }.to_string(),
        ...
        let venv_component = ComponentDiagnostic {
            ...
            status: if venv_status.is_active { "Active" } else { "Inactive" }.to_string(),
            version: None,
            path: venv_status.path.clone(),
            issues: self.get_component_issues("Virtual Environment"),
            details: if venv_status.is_active {
                format!("Virtual environment is active at {:?}", venv_status.path.as_deref().unwrap_or("unknown"))
            } else {
                format!("Virtual environment is not active. Expected at ./venv. Use: {}", venv_status.activation_command)
            },
        };
        ...
        let uv_component = ComponentDiagnostic {
            ...
            status: if self.uv_available { "Available" } else { "Missing" }.to_string(),
            version: self.uv_version.clone(),
            path: None,
            issues: self.get_component_issues("UV"),
            details: if self.uv_available {
                format!("UV {} is available", self.uv_version.as_deref().unwrap_or("unknown"))
            } else {
        ...
        let mineru_component = ComponentDiagnostic {
            ...
            status: if self.mineru_available { "Available" } else { "Missing" }.to_string(),
            version: self.mineru_version.clone(),
            path: None,
            issues: self.get_component_issues("MinerU"),
            details: if self.mineru_available {
                format!("MinerU {} is available", self.mineru_version.as_deref().unwrap_or("unknown"))
            } else {
        ...
        let markitdown_component = ComponentDiagnostic {
            ...
            status: if self.markitdown_available { "Available" } else { "Missing" }.to_string(),
            version: self.markitdown_version.clone(),
            path: None,
            issues: self.get_component_issues("MarkItDown"),
            details: if self.markitdown_available {
                format!("MarkItDown {} is available", self.markitdown_version.as_deref().unwrap_or("unknown"))
            } else {
        ...
        let cuda_component = ComponentDiagnostic {
            name: "CUDA".to_string(),
            status: if self.cuda_available { "Available" } else { "Not Available" }.to_string(),
            version: self.cuda_version.clone(),
            path: None,
            issues: self.get_component_issues("CUDA"),
            details: if self.cuda_available {
                format!("CUDA {} is available with {} device(s)", 
    ...
    fn generate_recommendations(&self, report: &mut DiagnosticReport) {
        ...
        if !self.python_available {
            report.recommendations.push("Install Python 3.8+ to enable document parsing functionality".to_string());
            report.next_steps.push("1. Install Python 3.8 or higher".to_string());
        }
        
        if !self.virtual_env_active {
            report.recommendations.push("Create and activate a virtual environment for isolated dependency management".to_string());
            report.next_steps.push("2. Run 'document-parser uv-init' to set up the environment".to_string());
        }
        
        if !self.uv_available {
            report.recommendations.push("Install UV tool for fast Python package management".to_string());
            if !report.next_steps.iter().any(|step| step.contains("uv-init")) {
                report.next_steps.push("2. Run 'document-parser uv-init' to install UV and set up dependencies".to_string());
            }
        }
        
        if !self.mineru_available {
            report.recommendations.push("Install MinerU for PDF document parsing capabilities".to_string());
            if !report.next_steps.iter().any(|step| step.contains("uv-init")) {
                report.next_steps.push("3. Install MinerU with: uv pip install magic-pdf[full]".to_string());
            }
        }
        
        if !self.markitdown_available {
            report.recommendations.push("Install MarkItDown for multi-format document parsing".to_string());
            if !report.next_steps.iter().any(|step| step.contains("uv-init")) {
                report.next_steps.push("4. Install MarkItDown with: uv pip install markitdown".to_string());
            }
        }
        
        if self.is_ready() {
            report.recommendations.push("Environment is ready! You can start the document parsing server".to_string());
            report.next_steps.push("Run 'document-parser server' to start the service".to_string());
        }
        
        // 添加CUDA相关推荐
        if !self.cuda_available && self.python_available {
            report.recommendations.push("Consider installing CUDA for improved PDF processing performance".to_string());
        }
        
        // 添加虚拟环境配置推荐
        if self.virtual_env_active && !self.is_virtual_env_properly_configured() {
            report.recommendations.push("Virtual environment detected but may not be in the expected location (./venv)".to_string());
        }
    }
    ...
    pub fn format_diagnostic_report(&self) -> String {
        ...
        output.push_str(&format!("=== Environment Diagnostic Report ===\n"));
    ...
    pub fn health_score(&self) -> u8 {
        ...
        // 基础组件检查 (60分)
        if self.python_available { score += 20; }
        if self.mineru_available { score += 20; }
        if self.markitdown_available { score += 20; }
        
        // 工具支持 (20分)
        if self.uv_available { score += 10; }
        if self.virtual_env_active { score += 10; }
        
        // CUDA支持 (10分)
        if self.has_cuda_support() { score += 10; }
        
        // 扣除问题分数 (最多扣30分)
        let issue_penalty = self.issues.iter()
...
impl EnvironmentManager {
    ...
    pub fn get_system_python_executable() -> Vec<String> {
        if cfg!(windows) {
            ...
            vec!["python.exe".to_string(), "python3.exe".to_string(), "py.exe".to_string()]
        } else {
    ...
    /// 测试虚拟环境激活（跨平台）
    pub async fn test_virtual_environment_activation(&self, venv_path: &Path) -> Result<bool, AppError> {
    ...
    /// 获取虚拟环境信息（跨平台）
    pub async fn get_virtual_environment_info(&self, venv_path: &Path) -> Result<VirtualEnvInfo, AppError> {
        let python_exe = Self::get_venv_python_path(venv_path);
        ...
        Ok(VirtualEnvInfo {
            ...
            activation_script,
            is_valid,
            platform: if cfg!(windows) { "windows".to_string() } else { "unix".to_string() },
        })
    }

    /// 获取跨平台环境变量设置
    pub fn get_cross_platform_env_vars(&self, venv_path: &Path) -> std::collections::HashMap<String, String> {
        let mut env_vars = std::collections::HashMap::new();
        
        if cfg!(windows) {
            // Windows环境变量
            env_vars.insert("VIRTUAL_ENV".to_string(), venv_path.to_string_lossy().to_string());
            env_vars.insert("PATH".to_string(), format!("{};{}", 
    ...
    /// 添加进度发送器到现有环境管理器
    pub fn with_progress_sender(mut self, progress_sender: mpsc::UnboundedSender<InstallProgress>) -> Self {
    ...
    pub async fn check_environment(&self) -> Result<EnvironmentStatus, AppError> {
        ...
        info!("环境检查完成，状态: ready={}, 健康评分: {}/100, 耗时: {:?}", 
              status.is_ready(), status.health_score(), status.check_duration);
        Ok(status)
    }
    ...
    pub async fn get_enhanced_dependency_report(&self) -> Result<String, AppError> {
        ...
        // 总体状态
        report.push_str(&format!("总体兼容性: {}\n", 
            if verification_result.overall_compatible { "✓ 兼容" } else { "✗ 不兼容" }));
        report.push_str("\n");
        
        // MinerU状态
        report.push_str("=== MinerU 状态 ===\n");
        let mineru = &verification_result.mineru_status;
        report.push_str(&format!("可用性: {}\n", if mineru.is_available { "✓ 可用" } else { "✗ 不可用" }));
        report.push_str(&format!("功能性: {}\n", if mineru.is_functional { "✓ 正常" } else { "✗ 异常" }));
        ...
        if let Some(ref compat) = mineru.compatibility {
            report.push_str(&format!("版本兼容性: {}\n", if compat.is_compatible { "✓ 兼容" } else { "✗ 不兼容" }));
        ...
        report.push_str(&format!("可用性: {}\n", if markitdown.is_available { "✓ 可用" } else { "✗ 不可用" }));
        report.push_str(&format!("功能性: {}\n", if markitdown.is_functional { "✓ 正常" } else { "✗ 异常" }));
    ...
    pub async fn diagnose_venv_path_issues(&self) -> Vec<String> {
        ...
        if !base_dir.exists() {
            ...
            if let Err(e) = self.check_directory_writable(base_dir).await {
                issues.push(format!("基础目录无写入权限: {} ({})", base_dir.display(), e));
            }
        }
        ...
        if venv_path.exists() {
            if !venv_path.is_dir() {
                issues.push(format!("虚拟环境路径存在但不是目录: {}", venv_path.display()));
            } else {
                // 检查虚拟环境完整性
                let python_exe = Self::get_venv_python_path(&venv_path);
                
                if !python_exe.exists() {
                    issues.push(format!("虚拟环境不完整，缺少Python可执行文件: {}", python_exe.display()));
                }
            }
        }
        ...
        if cfg!(windows) && venv_path.to_string_lossy().len() > 260 {
            issues.push(format!("虚拟环境路径过长 ({} 字符)，Windows限制为260字符", venv_path.to_string_lossy().len()));
        }
    ...
    pub async fn get_venv_recovery_suggestions(&self) -> Vec<String> {
        ...
        for issue in &issues {
            if issue.contains("不存在") {
                ...
                suggestions.push("1. 删除同名文件: rm venv (Unix) 或 del venv (Windows)".to_string());
    ...
    pub async fn auto_fix_venv_path_issues(&self) -> Result<Vec<String>, AppError> {
        ...
        if venv_path.exists() && !venv_path.is_dir() {
            match std::fs::remove_file(&venv_path) {
                Ok(_) => {
                    fixed_issues.push(format!("已删除阻碍虚拟环境创建的文件: {}", venv_path.display()));
                }
                Err(e) => {
    ...
    /// 验证虚拟环境状态并添加相关问题和警告
    fn validate_virtual_environment_status(&self, status: &mut EnvironmentStatus, python_info: &PythonInfo) {
        if !python_info.virtual_env_active {
            let issue = EnvironmentIssue {
                component: "Virtual Environment".to_string(),
                severity: IssueSeverity::High,
                message: "虚拟环境未激活".to_string(),
                suggestion: format!("创建并激活虚拟环境: 运行 'document-parser uv-init' 或手动运行 '{}'", 
            ...
            if let Some(ref venv_path) = python_info.virtual_env_path {
                let expected_venv_path = std::env::current_dir()
    ...
    fn get_python_installation_suggestion(&self, error_message: &str) -> String {
        if error_message.contains("command not found") || error_message.contains("not found") {
            ...
            format!("Python环境问题: {}。请检查Python安装并确保可以正常执行", error_message)
        }
    }
    ...
    fn get_uv_installation_suggestion(&self, error_message: &str) -> String {
        if error_message.contains("command not found") || error_message.contains("not found") {
            ...
            "UV版本不兼容。请更新到最新版本: curl -LsSf https://astral.sh/uv/install.sh | sh".to_string()
        } else {
            format!("UV工具问题: {}。请重新安装UV工具", error_message)
        }
    }
    ...
    fn get_mineru_installation_suggestion(&self, error_message: &str) -> String {
        if error_message.contains("command not found") || error_message.contains("not found") {
            ...
            format!("MinerU问题: {}。请检查安装或重新安装", error_message)
        }
    }
    ...
    fn get_markitdown_installation_suggestion(&self, error_message: &str) -> String {
        if error_message.contains("模块") || error_message.contains("module") {
            ...
            format!("MarkItDown问题: {}。请检查安装或重新安装", error_message)
        }
    }

    /// 带重试的Python环境检查
    async fn check_python_environment_with_retry(&self) -> Result<PythonInfo, AppError> {
        self.retry_with_backoff("Python环境检查", || self.check_python_environment()).await
    }
    
    /// 带重试的uv环境检查
    async fn check_uv_environment_with_retry(&self) -> Result<UvInfo, AppError> {
        self.retry_with_backoff("uv环境检查", || self.check_uv_environment()).await
    }
    
    /// 带重试的CUDA环境检查
    async fn check_cuda_environment_with_retry(&self) -> Result<CudaInfo, AppError> {
        self.retry_with_backoff("CUDA环境检查", || self.check_cuda_environment()).await
    }
    
    /// 带重试的MinerU环境检查
    async fn check_mineru_environment_with_retry(&self) -> Result<PackageInfo, AppError> {
        self.retry_with_backoff("MinerU环境检查", || self.check_mineru_environment()).await
    }
    
    /// 带重试的MarkItDown环境检查
    async fn check_markitdown_environment_with_retry(&self) -> Result<PackageInfo, AppError> {
        self.retry_with_backoff("MarkItDown环境检查", || self.check_markitdown_environment()).await
    }
    
    /// 验证依赖版本兼容性
    pub async fn verify_dependency_compatibility(&self) -> Result<DependencyVerificationResult, AppError> {
        debug!("开始依赖版本兼容性验证");
        ...
        let overall_compatible = mineru_status.is_available && 
                                ...
                                mineru_status.compatibility.as_ref().map_or(true, |c| c.is_compatible) &&
                                markitdown_status.compatibility.as_ref().map_or(true, |c| c.is_compatible);
        ...
        if let Some(ref compat) = mineru_status.compatibility {
            if !compat.is_compatible {
                critical_issues.push(format!("MinerU版本不兼容: {} (最低要求: {})", 
        ...
        if let Some(ref compat) = markitdown_status.compatibility {
            if !compat.is_compatible {
                critical_issues.push(format!("MarkItDown版本不兼容: {} (最低要求: {})", 
                    compat.current_version, compat.minimum_version));
            }
            if compat.upgrade_available {
                if let Some(ref rec) = compat.upgrade_recommendation {
    ...
    async fn verify_mineru_dependency(&self) -> Result<DependencyStatus, AppError> {
        ...
        match self.check_mineru_environment().await {
            Ok(package_info) => {
                ...
                // 验证版本兼容性
                status.compatibility = Some(self.check_mineru_version_compatibility(&package_info.version).await);
            }
            Err(e) => {
    ...
    async fn verify_markitdown_dependency(&self) -> Result<DependencyStatus, AppError> {
        ...
        match self.check_markitdown_environment().await {
            Ok(package_info) => {
                ...
                // 验证版本兼容性
                status.compatibility = Some(self.check_markitdown_version_compatibility(&package_info.version).await);
            }
            Err(e) => {
    ...
    /// 检查MinerU版本兼容性
    async fn check_mineru_version_compatibility(&self, current_version: &str) -> PackageCompatibility {
        ...
        if upgrade_available {
            upgrade_recommendation = Some(
                "升级MinerU到最新版本: uv pip install -U \"mineru[core]\"".to_string()
            );
        }
    ...
    /// 检查MarkItDown版本兼容性
    async fn check_markitdown_version_compatibility(&self, current_version: &str) -> PackageCompatibility {
        ...
        if upgrade_available {
            upgrade_recommendation = Some(
                "升级MarkItDown到最新版本: uv pip install -U markitdown".to_string()
            );
        }
    ...
    /// 通用重试机制
    async fn retry_with_backoff<T, F, Fut>(&self, operation_name: &str, mut operation: F) -> Result<T, AppError>
    where
        F: FnMut() -> Fut,
        Fut: std::future::Future<Output = Result<T, AppError>>,
    {
            ...
            match operation().await {
                ...
                Err(e) => {
                    ...
                    if attempt < self.retry_config.max_attempts {
                        warn!("{} 第{}次尝试失败，{}秒后重试", operation_name, attempt, delay.as_secs_f32());
                        ...
                        if let Some(sender) = &self.progress_sender {
                            let progress = InstallProgress {
                                ...
                                progress: (attempt as f32 / self.retry_config.max_attempts as f32) * 100.0,
                                message: format!("重试中... ({}/{})", attempt, self.retry_config.max_attempts),
                                estimated_time_remaining: Some(delay * (self.retry_config.max_attempts - attempt)),
        ...
        error!("{} 在{}次尝试后仍然失败", operation_name, self.retry_config.max_attempts);
        Err(last_error.unwrap_or_else(|| AppError::Environment(format!("{} 失败", operation_name))))
    }
    ...
    async fn check_python_environment(&self) -> Result<PythonInfo, AppError> {
        ...
        let output = timeout(self.timeout_duration, version_cmd)
            .await
            .map_err(|_| AppError::Environment(format!("Python版本检查超时: {}", self.timeout_duration.as_secs())))?
            .map_err(|e| AppError::Environment(format!("无法执行Python命令: {}", e)))?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(AppError::Environment(format!("Python命令执行失败: {}", stderr)));
        }
        ...
        let virtual_env_active = lines.get(0)
    ...
    /// 检查CUDA环境
    async fn check_cuda_environment(&self) -> Result<CudaInfo, AppError> {
        debug!("检查CUDA环境");
        ...
        debug!("CUDA环境检查完成: available={}, devices={}", !devices.is_empty(), devices.len());
    ...
    async fn check_mineru_environment(&self) -> Result<PackageInfo, AppError> {
        ...
        // 检查mineru命令是否可执行
        let help_cmd = Command::new(&mineru_path)
            .arg("--help")
            .output();
        ...
        let help_output = timeout(self.timeout_duration, help_cmd)
            ...
            .map_err(|e| AppError::Environment(format!("无法执行MinerU帮助命令: {}. 请确保已正确安装MinerU", e)))?;
 
        if !help_output.status.success() {
            let stderr = String::from_utf8_lossy(&help_output.stderr);
            return Err(AppError::Environment(format!("MinerU帮助命令执行失败: {}. 请检查MinerU安装", stderr)));
        }
        
        // 验证mineru命令功能性 - 测试基本功能
        let test_cmd = Command::new(&mineru_path)
            .arg("--version")
            .output();
        ...
        let version = match version_output {
            ...
            Err(e) => {
                return Err(AppError::Environment(format!("MinerU版本检查执行失败: {}. 请检查MinerU安装", e)));
            }
        };
    ...
    async fn check_markitdown_environment(&self) -> Result<PackageInfo, AppError> {
        ...
        if !import_output.status.success() {
            ...
            return Err(AppError::Environment(format!(
                "MarkItDown模块导入失败: {}. 请运行 'uv pip install markitdown' 安装MarkItDown", 
                stderr
            )));
        }
...
"#)
        ...
        match func_result {
            ...
            Ok(Ok(output)) => {
                ...
                return Err(AppError::Environment(format!(
                    "MarkItDown功能验证失败: {}. 请重新安装MarkItDown", stderr
                )));
            }
            Ok(Err(e)) => {
    ...
    pub async fn setup_python_environment(&self) -> Result<(), AppError> {
        ...
        // 发送开始进度
        self.send_progress("环境设置", InstallStage::Preparing, 0.0, "准备环境设置").await;
        
        // 确保基础目录存在
        self.ensure_base_directory().await?;
        self.send_progress("环境设置", InstallStage::Preparing, 10.0, "准备工作目录").await;
        
        // 检查并安装uv
        match self.is_uv_available().await? {
            UvAvailabilityStatus::Available { version, compatibility } => {
                if compatibility.is_compatible {
                    ...
                    warn!("uv版本不兼容，重新安装: {}", compatibility.recommendation.unwrap_or_default());
                    self.send_progress("环境设置", InstallStage::Installing, 20.0, "重新安装兼容版本的uv").await;
                    self.install_uv_with_progress().await?;
                }
            }
            UvAvailabilityStatus::IncompatibleVersion { version, issue } => {
                warn!("uv版本不兼容: {} - {}", version, issue);
                self.send_progress("环境设置", InstallStage::Installing, 20.0, "安装兼容版本的uv").await;
                self.install_uv_with_progress().await?;
            }
            UvAvailabilityStatus::ExecutionFailed { error } => {
                warn!("uv执行失败，重新安装: {}", error);
                self.send_progress("环境设置", InstallStage::Installing, 20.0, "重新安装uv工具").await;
                self.install_uv_with_progress().await?;
            }
            UvAvailabilityStatus::NotInstalled { error: _ } => {
                info!("uv工具未安装，开始安装");
                self.send_progress("环境设置", InstallStage::Installing, 20.0, "安装uv工具").await;
                self.install_uv_with_progress().await?;
            }
        }
        
        // 创建Python虚拟环境
        self.send_progress("环境设置", InstallStage::Configuring, 40.0, "创建Python虚拟环境").await;
        self.create_python_venv_with_progress().await?;
        
        // 安装依赖
        self.send_progress("环境设置", InstallStage::Installing, 60.0, "安装Python依赖").await;
        self.install_dependencies().await?;
        
        // 验证安装（非阻塞）
        self.send_progress("环境设置", InstallStage::Verifying, 90.0, "验证环境").await;
        match self.validate_engines().await {
            Ok(is_valid) => {
                if is_valid {
                    self.send_progress("环境设置", InstallStage::Completed, 100.0, "环境设置完成").await;
                    info!("Python环境设置完成");
                } else {
                    warn!("环境验证未完全通过，但安装过程已完成");
                    self.send_progress("环境设置", InstallStage::Completed, 100.0, "安装完成（部分验证待完善）").await;
                }
            }
            Err(e) => {
                warn!("环境验证过程出现问题: {}", e);
                self.send_progress("环境设置", InstallStage::Completed, 100.0, "安装完成（验证待重试）").await;
            }
        }
    ...
    /// 发送安装进度
    async fn send_progress(&self, package: &str, stage: InstallStage, progress: f32, message: &str) {
        if let Some(sender) = &self.progress_sender {
    ...
    /// 检查UV版本兼容性
    fn check_uv_version_compatibility(&self, version_str: &str) -> Result<UvVersionCompatibility, AppError> {
        // UV最低版本要求：0.1.0
        let minimum_version = "0.1.0";
        
        // 解析版本号
        let current_version = self.extract_uv_version(version_str)
            .ok_or_else(|| AppError::Environment(format!("无法解析uv版本: {}", version_str)))?;
        
        let min_version = self.parse_version_tuple(minimum_version)
            .ok_or_else(|| AppError::Environment(format!("无法解析最低版本要求: {}", minimum_version)))?;
        
        let is_compatible = current_version >= min_version;
        
        let recommendation = if !is_compatible {
            Some(format!("请升级uv到{}或更高版本，运行: curl -LsSf https://astral.sh/uv/install.sh | sh", minimum_version))
    ...
    pub async fn install_uv_with_progress(&self) -> Result<(), AppError> {
        ...
        self.send_progress("uv", InstallStage::Preparing, 0.0, "准备安装uv工具").await;
        ...
        match install_result {
            Ok(_) => {
                self.send_progress("uv", InstallStage::Verifying, 90.0, "验证uv安装").await;
                
                // 验证安装
                match self.is_uv_available().await? {
                    UvAvailabilityStatus::Available { version, compatibility } => {
                        if compatibility.is_compatible {
                            self.send_progress("uv", InstallStage::Completed, 100.0, 
                                &format!("uv安装完成: {}", version)).await;
                            info!("uv安装成功: {}", version);
                            Ok(())
                        } else {
                            let error_msg = format!("uv版本不兼容: {}", compatibility.recommendation.unwrap_or_default());
                            self.send_progress("uv", InstallStage::Failed(error_msg.clone()), 0.0, "版本不兼容").await;
                    ...
                    UvAvailabilityStatus::ExecutionFailed { error } => {
                        ...
                        self.send_progress("uv", InstallStage::Failed(error_msg.clone()), 0.0, "执行失败").await;
                        Err(AppError::Environment(error_msg))
                    }
                    UvAvailabilityStatus::NotInstalled { error } => {
                        let error_msg = format!("uv安装后仍不可用: {}", error);
                        self.send_progress("uv", InstallStage::Failed(error_msg.clone()), 0.0, "安装失败").await;
                        Err(AppError::Environment(error_msg))
                    }
                }
            }
    ...
    async fn install_uv_with_curl_script(&self) -> Result<(), AppError> {
        self.send_progress("uv", InstallStage::Downloading, 10.0, "下载uv安装脚本").await;
        ...
        self.send_progress("uv", InstallStage::Installing, 50.0, "执行curl安装脚本").await;
    ...
    async fn install_uv_with_powershell_script(&self) -> Result<(), AppError> {
        self.send_progress("uv", InstallStage::Downloading, 10.0, "下载uv PowerShell脚本").await;
        ...
        self.send_progress("uv", InstallStage::Installing, 50.0, "执行PowerShell安装脚本").await;
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(AppError::Environment(format!("PowerShell安装uv失败: {}", stderr)));
        }
    ...
    async fn install_uv_with_pip(&self) -> Result<(), AppError> {
        self.send_progress("uv", InstallStage::Installing, 30.0, "使用pip安装uv").await;
        
        let install_cmd = Command::new("pip")
            .arg("install")
            .arg("uv")
            .output();
    ...
    async fn install_uv_with_system_package_manager(&self) -> Result<(), AppError> {
        self.send_progress("uv", InstallStage::Installing, 30.0, "使用系统包管理器安装uv").await;
        ...
        for (manager, args) in package_managers {
            if self.is_command_available(manager).await {
                let install_cmd = Command::new(manager)
                    .args(&args)
                    .output();
        ...
        Err(AppError::Environment("所有系统包管理器都无法安装uv".to_string()))
    }
    ...
    async fn try_fallback_uv_installation(&self) -> Result<(), AppError> {
        ...
        for method in fallback_methods {
            ...
            let result = match method {
                ...
                UvInstallationMethod::SystemPackageManager => self.install_uv_with_system_package_manager().await,
                UvInstallationMethod::PowerShellScript => self.install_uv_with_powershell_script().await,
            };
    ...
    fn handle_venv_creation_error(&self, error: &str, venv_path: &Path) -> AppError {
        ...
        } else if error_lower.contains("space") || error_lower.contains("空间") || error_lower.contains("disk") {
    ...
    async fn cleanup_corrupted_venv(&self, venv_path: &Path) -> Result<(), AppError> {
        ...
        // 检查是否有权限删除
        if let Err(e) = self.check_directory_writable(venv_path.parent().unwrap_or(venv_path)).await {
            return Err(AppError::permission_error(
                format!("无权限清理虚拟环境: {}", e),
                venv_path
            ));
    ...
    /// 验证当前目录设置（任务12的核心功能）
    #[instrument(skip(self))]
    pub async fn validate_current_directory_setup(&self) -> Result<DirectoryValidationResult, AppError> {
                ...
                if !python_exe.exists() {
                    ...
                    match self.test_virtual_environment_activation(&venv_path).await {
                        ...
                        Ok(false) => {
                            ...
                            result.cleanup_options.push(CleanupOption {
                                option_type: CleanupType::RemoveCorruptedVenv,
                                description: format!("清理无法激活的虚拟环境: {}", venv_path.display()),
                                risk_level: CleanupRisk::Medium,
                                command: format!("rm -rf {}", venv_path.display()),
                            });
                        }
        ...
        if cfg!(windows) && venv_path.to_string_lossy().len() > 260 {
            ...
            result.issues.push(DirectoryValidationIssue {
                ...
                message: format!("虚拟环境路径过长 ({} 字符)，Windows限制为260字符", 
        ...
        info!("目录验证完成: valid={}, issues={}, warnings={}", 
              result.is_valid, result.issues.len(), result.warnings.len());
    ...
    fn generate_directory_recommendations(&self, result: &mut DirectoryValidationResult) {
        if result.is_valid && result.warnings.is_empty() {
            result.recommendations.push("当前目录设置良好，可以安全创建虚拟环境".to_string());
            return;
        }
        
        if !result.is_valid {
            result.recommendations.push("请先解决关键问题后再创建虚拟环境".to_string());
        }
        ...
        for issue in &result.issues {
            match issue.issue_type {
                DirectoryIssueType::PermissionDenied => {
                    if cfg!(unix) {
                        result.recommendations.push("使用 'chmod 755 .' 修改目录权限".to_string());
                        result.recommendations.push("使用 'chown $USER .' 修改目录所有者".to_string());
                    } else if cfg!(windows) {
                        result.recommendations.push("以管理员身份运行命令".to_string());
                        result.recommendations.push("检查Windows用户账户控制(UAC)设置".to_string());
                    }
                }
                DirectoryIssueType::InsufficientSpace => {
                    result.recommendations.push("清理不需要的文件释放磁盘空间".to_string());
                    result.recommendations.push("考虑移动项目到有更多可用空间的磁盘".to_string());
                }
                DirectoryIssueType::PathConflict => {
                    result.recommendations.push("删除或重命名冲突的文件/目录".to_string());
                }
                DirectoryIssueType::PathTooLong => {
                    result.recommendations.push("移动项目到路径较短的目录".to_string());
                    result.recommendations.push("使用较短的目录名称".to_string());
                }
            }
        }
        ...
        if !result.cleanup_options.is_empty() {
            result.recommendations.push("可以使用以下清理选项解决问题:".to_string());
            for option in &result.cleanup_options {
                result.recommendations.push(format!("  - {} (风险: {:?})", 
    ...
    /// 执行自动清理选项
    pub async fn execute_cleanup_option(&self, option_type: CleanupType) -> Result<String, AppError> {
        let venv_path = Path::new(&self.base_dir).join("venv");
        
        match option_type {
            CleanupType::RemoveConflictingFile => {
                if venv_path.exists() && venv_path.is_file() {
                    std::fs::remove_file(&venv_path)
                        .map_err(|e| AppError::permission_error(
                            format!("删除冲突文件失败: {}", e), &venv_path))?;
                    Ok(format!("成功删除冲突文件: {}", venv_path.display()))
                } else {
                    Err(AppError::path_error("冲突文件不存在".to_string(), &venv_path))
                }
            }
            CleanupType::RemoveCorruptedVenv => {
                if venv_path.exists() && venv_path.is_dir() {
                    ...
                    Err(AppError::path_error("虚拟环境目录不存在".to_string(), &venv_path))
                }
            }
            CleanupType::CreateBackup => {
                if venv_path.exists() {
                    let backup_path = Path::new(&self.base_dir).join("venv.backup");
                    std::fs::rename(&venv_path, &backup_path)
                        .map_err(|e| AppError::permission_error(
                            format!("创建备份失败: {}", e), &venv_path))?;
                    Ok(format!("成功备份虚拟环境到: {}", backup_path.display()))
                } else {
                    Err(AppError::path_error("虚拟环境不存在，无需备份".to_string(), &venv_path))
                }
            }
        }
    }
    ...
    async fn create_python_venv_with_progress(&self) -> Result<(), AppError> {
        ...
        // 预检查：验证创建条件
        self.send_progress("虚拟环境", InstallStage::Preparing, 5.0, "验证创建条件").await;
        if let Err(e) = self.validate_venv_creation_preconditions(&venv_path).await {
            self.send_progress("虚拟环境", InstallStage::Failed(e.to_string()), 0.0, "前置条件检查失败").await;
            return Err(e);
        }
        ...
        if venv_path.exists() && venv_path.is_dir() {
            ...
            if python_exe.exists() {
                ...
                self.send_progress("虚拟环境", InstallStage::Completed, 100.0, "虚拟环境已存在").await;
                return Ok(());
            } else {
                warn!("检测到损坏的虚拟环境，尝试清理");
                self.send_progress("虚拟环境", InstallStage::Preparing, 10.0, "清理损坏的虚拟环境").await;
                self.cleanup_corrupted_venv(&venv_path).await?;
            }
        }
        
        info!("创建Python虚拟环境: {}", venv_path.display());
        self.send_progress("虚拟环境", InstallStage::Preparing, 15.0, "准备创建虚拟环境").await;
        ...
        self.send_progress("虚拟环境", InstallStage::Installing, 50.0, "创建虚拟环境").await;
        
        let output = timeout(Duration::from_secs(120), create_cmd)
            .await
            .map_err(|_| {
                self.handle_venv_creation_error("虚拟环境创建超时", &venv_path)
            })?
            .map_err(|e| {
        ...
        if !output.status.success() {
            ...
            self.send_progress("虚拟环境", InstallStage::Failed(error_msg.clone()), 0.0, "创建失败").await;
        ...
        self.send_progress("虚拟环境", InstallStage::Verifying, 90.0, "验证虚拟环境").await;
        
        // 验证虚拟环境创建结果
        if let Err(e) = self.verify_venv_creation(&venv_path).await {
            self.send_progress("虚拟环境", InstallStage::Failed(e.to_string()), 0.0, "验证失败").await;
            return Err(e);
        }
        
        self.send_progress("虚拟环境", InstallStage::Completed, 100.0, "虚拟环境创建完成").await;
        info!("Python虚拟环境创建完成");
        Ok(())
    }
    ...
    async fn install_mineru_with_progress(&self) -> Result<(), AppError> {
        ...
        self.send_progress("MinerU", InstallStage::Preparing, 0.0, "准备安装MinerU").await;
        ...
        self.send_progress("MinerU", InstallStage::Downloading, 20.0, "下载MinerU包").await;
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            self.send_progress("MinerU", InstallStage::Failed(stderr.to_string()), 0.0, "安装失败").await;
            return Err(AppError::Environment(format!("MinerU安装失败: {stderr}")));
        }
        
        self.send_progress("MinerU", InstallStage::Configuring, 80.0, "配置MinerU环境").await;
        ...
        self.send_progress("MinerU", InstallStage::Verifying, 90.0, "验证MinerU安装").await;
        
        // 验证安装
        match self.check_mineru_environment().await {
            Ok(_) => {
                self.send_progress("MinerU", InstallStage::Completed, 100.0, "MinerU安装完成").await;
                info!("MinerU安装完成");
                Ok(())
            }
            Err(e) => {
                self.send_progress("MinerU", InstallStage::Failed(e.to_string()), 0.0, "验证失败").await;
    ...
    async fn install_markitdown_with_progress(&self) -> Result<(), AppError> {
        ...
        self.send_progress("MarkItDown", InstallStage::Preparing, 0.0, "准备安装MarkItDown").await;
        ...
        self.send_progress("MarkItDown", InstallStage::Downloading, 20.0, "下载MarkItDown包").await;
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            self.send_progress("MarkItDown", InstallStage::Failed(stderr.to_string()), 0.0, "安装失败").await;
            return Err(AppError::Environment(format!("MarkItDown安装失败: {stderr}")));
        }
        
        self.send_progress("MarkItDown", InstallStage::Verifying, 90.0, "验证MarkItDown安装").await;
        
        // 验证安装
        match self.check_markitdown_environment().await {
            Ok(_) => {
                self.send_progress("MarkItDown", InstallStage::Completed, 100.0, "MarkItDown安装完成").await;
                info!("MarkItDown安装完成");
                Ok(())
            }
            Err(e) => {
                self.send_progress("MarkItDown", InstallStage::Failed(e.to_string()), 0.0, "验证失败").await;
                Err(AppError::Environment(format!("MarkItDown安装验证失败: {e}")))
            }
        }
    }
    ...
    async fn configure_mineru_model_source(&self) -> Result<(), AppError> {
        ...
        let config_dir = format!("{}/.mineru", home_dir);
    ...
    "default_source": "modelscope"
        ...
        if let Err(e) = std::fs::write(&config_file, config_content) {
            return Err(AppError::Environment(format!("写入MinerU配置文件失败: {}", e)));
        }
    ...
    pub async fn validate_environment(&self) -> Result<bool, AppError> {
        ...
        if !is_valid {
            ...
            for issue in status.get_critical_issues() {
                error!("关键问题 [{}]: {} - {}", issue.component, issue.message, issue.suggestion);
            }
        } else {
    ...
    pub async fn generate_environment_report(&self) -> Result<String, AppError> {
        ...
        if !status.cuda_devices.is_empty() {
            ...
            for device in &status.cuda_devices {
                report.push_str(&format!("    - GPU {}: {} ({}MB 可用)\n", 
                    device.id, device.name, device.memory_free / 1024 / 1024));
            }
        }
        
        report.push_str(&format!("MinerU: {} ({:?})\n", 
            if status.mineru_available { "✓" } else { "✗" },
            status.mineru_version.as_deref().unwrap_or("未安装")));
        
        report.push_str(&format!("MarkItDown: {} ({:?})\n", 
            if status.markitdown_available { "✓" } else { "✗" },
    ...
    pub async fn check_current_directory_readiness(&self) -> Result<DirectoryValidationResult, AppError> {
        ...
        if !result.cleanup_options.is_empty() {
            ...
            for (i, option) in result.cleanup_options.iter().enumerate() {
                report.push_str(&format!("{}. {} (风险: {:?})\n", 
                                       i + 1, option.description, option.risk_level));
...
mod tests {
    ...
    async fn test_for_current_directory_factory() {
        ...
        // 验证Python路径根据平台正确设置
        let expected_python_path = EnvironmentManager::get_venv_python_path(&current_dir.join("venv"));
        assert_eq!(manager.python_path, expected_python_path.to_string_lossy().to_string());
    ...
    async fn test_uv_version_parsing() {
        ...
        // 测试版本解析
        assert_eq!(env_manager.extract_uv_version("uv 0.4.15"), Some((0, 4, 15)));
    ...
    async fn test_cross_platform_path_functions() {
        ...
        if cfg!(windows) {
            assert_eq!(activation_script, venv_path.join("Scripts").join("activate.bat"));
        } else {
    ...
    async fn test_cross_platform_environment_variables() {
        ...
        // 验证VIRTUAL_ENV变量
        assert_eq!(env_vars.get("VIRTUAL_ENV").unwrap(), &venv_path.to_string_lossy().to_string());
    ...
    async fn test_virtual_environment_activation_commands() {
        ...
        if cfg!(windows) {
            assert_eq!(powershell_cmd, Some(".\\venv\\Scripts\\Activate.ps1".to_string()));
        } else {
    ...
    async fn test_virtual_environment_info() {
        ...
        // 验证平台特定路径
        if cfg!(windows) {
            assert_eq!(venv_info.python_executable, venv_path.join("Scripts").join("python.exe"));
            assert_eq!(venv_info.pip_executable, venv_path.join("Scripts").join("pip.exe"));
            assert_eq!(venv_info.activation_script, venv_path.join("Scripts").join("activate.bat"));
            assert_eq!(venv_info.platform, "windows");
        } else {
            assert_eq!(venv_info.python_executable, venv_path.join("bin").join("python"));
            assert_eq!(venv_info.pip_executable, venv_path.join("bin").join("pip"));
            assert_eq!(venv_info.activation_script, venv_path.join("bin").join("activate"));
