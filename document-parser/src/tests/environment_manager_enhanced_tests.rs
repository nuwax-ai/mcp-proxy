#[cfg(test)]
mod tests {
    use crate::utils::environment_manager::{EnvironmentManager, EnvironmentStatus, VirtualEnvStatus, DiagnosticReport};
    ...
    async fn test_enhanced_status_methods() {
        ...
        let venv_status = env_manager.check_virtual_environment_status().await.unwrap();
        assert!(!venv_status.activation_command.is_empty());
    }
    ...
    async fn test_directory_validation() {
        ...
        let validation_result = env_manager.check_current_directory_readiness().await.unwrap();
        
        // Verify validation result structure
        assert!(validation_result.current_directory.exists());
        assert!(validation_result.venv_path.to_string_lossy().ends_with("venv"));
    ...
    async fn test_cleanup_options() {
        ...
        let env_manager = EnvironmentManager::new(
            temp_path.join("venv").join("bin").join("python").to_string_lossy().to_string(),
        ...
        let result = env_manager.execute_cleanup_option(CleanupType::RemoveConflictingFile).await;
