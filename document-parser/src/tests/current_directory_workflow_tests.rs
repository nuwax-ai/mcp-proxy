...
mod tests {
    ...
    use crate::utils::environment_manager::{
        EnvironmentManager, EnvironmentStatus, VirtualEnvInfo
    };
    use crate::{AppState, AppConfig};
    use crate::models::{DocumentTask, DocumentFormat, ParserEngine, TaskStatus, SourceType};
    ...
    impl CurrentDirectoryTestEnvironment {
        ...
        async fn create_mock_venv(&self) -> Result<(), Box<dyn std::error::Error>> {
            ...
            if cfg!(windows) {
                ...
                fs::write(venv_path.join("Scripts").join("activate.bat"), "mock activate").await?;
                ...
                // 创建Python可执行文件（模拟）
                fs::write(venv_path.join("bin").join("python"), "#!/bin/bash\necho 'Mock Python 3.9.0'").await?;
                fs::write(venv_path.join("bin").join("pip"), "#!/bin/bash\necho 'Mock pip'").await?;
                fs::write(venv_path.join("bin").join("activate"), "# Mock activate script").await?;
                fs::write(venv_path.join("bin").join("mineru"), "#!/bin/bash\necho 'Mock MinerU'").await?;
                
                // 设置执行权限
                #[cfg(unix)]
                {
                    use std::os::unix::fs::PermissionsExt;
                    let mut perms = fs::metadata(venv_path.join("bin").join("python")).await?.permissions();
                    perms.set_mode(0o755);
                    fs::set_permissions(venv_path.join("bin").join("python"), perms).await?;
                    
                    let mut perms = fs::metadata(venv_path.join("bin").join("mineru")).await?.permissions();
                    perms.set_mode(0o755);
        ...
        /// 验证虚拟环境是否在当前目录
        async fn verify_venv_in_current_directory(&self) -> Result<bool, Box<dyn std::error::Error>> {
            let venv_path = self.get_venv_path();
    ...
    async fn test_uv_init_creates_venv_in_current_directory() {
        let test_env = CurrentDirectoryTestEnvironment::new().await
        ...
        // 检查当前目录设置
        let validation_result = test_env.env_manager.check_current_directory_readiness().await
        ...
        let expected_venv = test_env.get_venv_path().canonicalize().unwrap_or(test_env.get_venv_path());
        let actual_venv = validation_result.venv_path.canonicalize().unwrap_or(validation_result.venv_path.clone());
        assert_eq!(actual_venv, expected_venv);
        
        // 模拟uv-init过程：创建虚拟环境
        test_env.create_mock_venv().await
            .expect("Failed to create mock virtual environment");
        
        // 验证虚拟环境在当前目录下创建成功
        assert!(test_env.verify_venv_in_current_directory().await
        ...
        assert!(venv_path.file_name().unwrap() == "venv", "Virtual environment should be named 'venv'");
        ...
        // 规范化路径以避免符号链接问题
        let expected_canonical = expected_python_path.canonicalize().unwrap_or(expected_python_path);
        let actual_canonical = actual_python_path.canonicalize().unwrap_or(actual_python_path);
        
        assert!(actual_canonical.exists(), "Python executable should exist in venv");
        
        if cfg!(windows) {
            assert!(actual_canonical.to_string_lossy().ends_with("Scripts\\python.exe"));
        } else {
    ...
    async fn test_virtual_environment_info_detection() {
        ...
        // 获取虚拟环境信息
        let venv_info = test_env.env_manager.get_virtual_environment_info(&test_env.get_venv_path()).await
        ...
        if cfg!(windows) {
            ...
            assert!(venv_info.activation_script.to_string_lossy().contains("Scripts"));
            assert_eq!(venv_info.platform, "windows");
        } else {
            assert!(venv_info.python_executable.to_string_lossy().contains("bin"));
            assert!(venv_info.activation_script.to_string_lossy().contains("bin"));
    ...
    async fn test_server_startup_finds_correct_virtual_environment() {
        ...
        // 模拟服务器启动时的环境检测
        let env_status = test_env.env_manager.check_environment().await
        ...
        if let Some(ref python_path) = env_status.python_path {
            assert!(python_path.contains("venv"), "Python path should point to virtual environment: {}", python_path);
            
            if cfg!(windows) {
                assert!(python_path.contains("Scripts"), "Windows Python path should contain Scripts");
            } else {
                assert!(python_path.contains("bin"), "Unix Python path should contain bin");
            }
        }
        ...
        // 创建应用状态来模拟服务器启动
        let app_state = test_env.create_test_app_state().await
    ...
    async fn test_mineru_command_path_detection() {
        ...
        // 验证MinerU可执行文件路径
        assert!(mineru_path.exists(), "MinerU executable should exist in venv");
        
        if cfg!(windows) {
            assert!(mineru_path.to_string_lossy().ends_with("Scripts\\mineru.exe"));
        } else {
    ...
    async fn test_environment_status_includes_current_directory_info() {
        ...
        assert!(venv_component.details.contains("venv") || venv_component.details.contains("./venv"));
    ...
    async fn test_cross_platform_venv_path_handling() {
        ...
        if cfg!(windows) {
            ...
            assert!(activation_script.to_string_lossy().ends_with("activate.bat"));
        } else {
    ...
    async fn test_current_directory_validation_and_cleanup() {
        ...
        fs::write(&venv_file_path, "conflicting file").await
        ...
        assert!(!validation_result.cleanup_options.is_empty(), "Should have cleanup options");
        
        // 验证清理选项
        let cleanup_options = &validation_result.cleanup_options;
        assert!(cleanup_options.iter().any(|opt| 
            matches!(opt.option_type, crate::utils::environment_manager::CleanupType::RemoveConflictingFile)
        ));
        
        // 测试清理功能
        use crate::utils::environment_manager::CleanupType;
        let cleanup_result = test_env.env_manager.execute_cleanup_option(CleanupType::RemoveConflictingFile).await;
        
        match cleanup_result {
            Ok(message) => {
                assert!(message.contains("成功删除冲突文件") || message.contains("successfully"));
                assert!(!venv_file_path.exists(), "Conflicting file should be removed");
            }
            Err(e) => {
    ...
    async fn test_environment_manager_factory_methods() {
        ...
        let expected_python_path = EnvironmentManager::get_venv_python_path(&expected_venv_path);
        ...
        let env_manager_with_progress = EnvironmentManager::for_current_directory_with_progress(progress_tx)
            .expect("Failed to create environment manager with progress");
        
        // 验证带进度跟踪的环境管理器也能正常工作
        let env_status_with_progress = env_manager_with_progress.check_environment().await
        ...
        assert_eq!(venv_status_with_progress.expected_path.as_deref(), Some("./venv"));
