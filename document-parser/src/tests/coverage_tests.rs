...
use super::test_config::{TestEnvironment, assertions, generators};
use crate::models::*;
...
mod coverage_tests {
    ...
    async fn test_complete_document_processing_pipeline() {
        ...
        let storage_service =
            Arc::new(StorageService::new(db.clone()).expect("Failed to create storage service"));
        let task_service =
        ...
        task_service
            .update_task_status(&task.id, completed_status)
            .await
            .expect("Failed to complete task");

        let final_task = task_service
            .get_task(&task.id)
            .await
    ...
    async fn test_error_handling_coverage() {
        ...
        for (i, error) in error_scenarios.into_iter().enumerate() {
            let task = task_service
                ...
                .await
                .expect("Failed to create error test task");

            let failed_status = TaskStatus::new_failed(error.clone(), 1);
            task_service
                .update_task_status(&task.id, failed_status)
                .await
                .expect("Failed to set task to failed");

            let failed_task = task_service
                .get_task(&task.id)
                .await
                ...
                Ok(doc_structure) => {
                    ...
                    match name {
                        ...
                        "with_images" => {
                            ...
                            assert!(doc_structure.toc.iter().any(|s| {
                                s.content_preview
                                    .as_ref()
                                    .map_or(false, |c| c.contains("![Image]"))
                            }));
                        }
                        "with_links" => {
                            assert!(doc_structure.toc.len() >= 1);
                            // Should preserve links
                            assert!(doc_structure.toc.len() >= 1); // Basic structure validation
                        }
                        "complex" => {
                            assert!(doc_structure.toc.len() >= 3);
                            // Should have multiple levels
                            let levels: std::collections::HashSet<_> =
                                doc_structure.toc.iter().map(|s| s.level).collect();
                            assert!(levels.len() >= 2);
                        }
                        _ => {
    ...
    async fn test_storage_operations_coverage() {
        ...
        // Read
        let retrieved_task = storage_service
            .get_task(&task.id)
            .await
            .expect("Failed to get task")
            .expect("Task not found");
        ...
        let updated_task = storage_service
            .get_task(&task.id)
            .await
        ...
        storage_service
            .save_task(&expired_task)
            .await
            .expect("Failed to save expired task");

        let cleaned_count = storage_service
    ...
    async fn test_format_detection_coverage() {
        ...
        for (filename, expected_format, _expected_engine) in test_cases {
            ...
            if detection_result.is_ok() {
                ...
                assert!(
                    dual_parser.supports_format(&result.format),
                    "Dual parser should support format for {}",
                    filename
                );
            }
        }
        ...
        for (filename, mime_type, expected_format) in mime_test_cases {
            ...
            let detection_result =
                format_detector.detect_format(test_file.to_str().unwrap(), mime_type);
            if detection_result.is_ok() {
                let result = detection_result.unwrap();
                assert_eq!(
                    result.format, expected_format,
                    "MIME type detection failed for {}",
                    filename
                );
            }
        }
    ...
    async fn test_task_queue_coverage() {
        ...
        let tasks = vec![
            ...
            generators::test_document_task_with_params(
                SourceType::Url,
                DocumentFormat::Word,
                ParserEngine::MarkItDown,
            ),
        ...
        assert!(
            ...
            "Queue service should shutdown gracefully"
        );
    }
    ...
    async fn test_image_processing_coverage() {
        ...
        let result = image_processor.batch_upload_images(image_paths).await;
        // Note: This will likely fail in test environment without OSS service
...
"#;
        ...
        let extracted_paths = ImageProcessor::extract_image_paths(markdown_with_images);
    ...
    async fn test_utility_functions_coverage() {
        ...
        let format =
            crate::utils::detect_format_from_path("test.pdf").expect("Failed to detect format");
...
mod integration_coverage_tests {
    ...
    async fn test_end_to_end_document_processing() {
        ...
        let task = task_service
            ...
            .await
            .expect("Failed to create integration task");
        ...
        let format_detector = FormatDetector::new();
        let detection_result =
            format_detector.detect_format(test_file.to_str().unwrap(), Some("application/pdf"));
        ...
        for (i, stage) in processing_stages.iter().enumerate() {
            ...
            let progress_details =
                ProgressDetails::new(format!("Processing stage: {}", stage.get_name()));
        ...
        task_service
            .update_task_status(&task.id, completion_status)
            .await
    ...
    async fn test_error_recovery_integration() {
        ...
        let task = task_service
            ...
            .await
            .expect("Failed to create error recovery task");
        ...
        if failed_task.retry_count < failed_task.max_retries {
            ...
            task_service
                .update_task_status(&task.id, retry_status)
                .await
                .expect("Failed to retry task");

            let retried_task = task_service
                .get_task(&task.id)
                .await
