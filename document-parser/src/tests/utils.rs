...
mod file_utils_tests {
    ...
    async fn test_filename_operations() {
        // 测试文件扩展名获取
        assert_eq!(
            get_file_extension("normal_file.txt"),
            Some("txt".to_string())
        );
        assert_eq!(
            get_file_extension("file with spaces.pdf"),
            Some("pdf".to_string())
        );
        assert_eq!(get_file_extension("file.docx"), Some("docx".to_string()));
        assert_eq!(
            get_file_extension("中文文件名.pdf"),
            Some("pdf".to_string())
        ); // 保留中文
...
mod format_utils_tests {
    ...
    async fn test_detect_format_from_path() {
        // 测试基于文件扩展名的格式检测
        assert_eq!(
            detect_format_from_path("test.pdf").unwrap(),
            DocumentFormat::PDF
        );
        assert_eq!(
            detect_format_from_path("document.docx").unwrap(),
            DocumentFormat::Word
        );
        assert_eq!(
            detect_format_from_path("document.doc").unwrap(),
            DocumentFormat::Word
        );
        assert_eq!(
            detect_format_from_path("presentation.pptx").unwrap(),
            DocumentFormat::PowerPoint
        );
        assert_eq!(
            detect_format_from_path("presentation.ppt").unwrap(),
            DocumentFormat::PowerPoint
        );
        assert_eq!(
            detect_format_from_path("spreadsheet.xlsx").unwrap(),
            DocumentFormat::Excel
        );
        assert_eq!(
            detect_format_from_path("spreadsheet.xls").unwrap(),
            DocumentFormat::Excel
        );
        assert_eq!(
            detect_format_from_path("image.png").unwrap(),
            DocumentFormat::Image
        );
        assert_eq!(
            detect_format_from_path("image.jpg").unwrap(),
            DocumentFormat::Image
        );
        assert_eq!(
            detect_format_from_path("image.jpeg").unwrap(),
            DocumentFormat::Image
        );
        // 对于未知扩展名，函数返回 DocumentFormat::Other 而不是错误
        assert!(matches!(
            detect_format_from_path("unknown.xyz").unwrap(),
...
mod validation_utils_tests {
    ...
    async fn test_file_extension_validation() {
        ...
        assert_eq!(
            get_file_extension("presentation.pptx"),
            Some("pptx".to_string())
        );
        assert_eq!(get_file_extension("image_001.png"), Some("png".to_string()));
        assert_eq!(
            get_file_extension("report-2023.docx"),
            Some("docx".to_string())
        );
