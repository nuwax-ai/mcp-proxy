...
impl StructuredSection {
    ...
    pub fn validate(&self) -> Result<(), AppError> {
        ...
        if self.content.len() > MAX_CONTENT_SIZE {
            return Err(AppError::Validation(
                format!("章节内容大小 {} 字节超过最大限制 {} 字节", 
                    self.content.len(), MAX_CONTENT_SIZE)
            ));
        }
    ...
    pub fn add_child(&mut self, child: StructuredSection) -> Result<(), AppError> {
        ...
        if child.level <= self.level {
            return Err(AppError::Validation(
                format!("子章节级别 {} 必须大于父章节级别 {}", child.level, self.level)
            ));
        }
    ...
    pub fn get_depth(&self) -> usize {
        if self.children.is_empty() {
            ...
            1 + self.children.iter().map(|c| c.get_depth()).max().unwrap_or(1)
        }
    }
    ...
    pub fn get_path(&self) -> String {
        if self.level == 1 {
            ...
            format!("{} > {}", "  ".repeat((self.level - 1) as usize), self.title)
        }
    }
    ...
    pub fn update_content(&mut self, new_content: String) -> Result<bool, AppError> {
        ...
        let content_changed = self.content_hash.map_or(true, |hash| hash != new_hash);
        ...
        if content_changed {
            ...
            if new_content.len() > MAX_CONTENT_SIZE {
                return Err(AppError::Validation(
                    format!("内容大小 {} 字节超过最大限制 {} 字节", 
                        new_content.len(), MAX_CONTENT_SIZE)
                ));
            }
...
impl StructuredDocument {
    ...
    fn collect_all_section_ids(
        ...
        for section in sections {
            if !seen_ids.insert(section.id.clone()) {
                return Err(AppError::Validation(
                    format!("重复的章节ID: {}", section.id)
                ));
            }
            self.collect_all_section_ids(&section.children, seen_ids)?;
        }
        Ok(())
    }
    ...
    fn build_section_index(&mut self, sections: &[StructuredSection], base_index: usize) {
        ...
        for (i, section) in sections.iter().enumerate() {
            ...
            if !section.children.is_empty() {
                self.build_section_index(&section.children, section_index * 1000);
            }
        }
    }
    ...
    fn calculate_section_word_count(&self, section: &StructuredSection) -> usize {
        ...
        let children_count = section.children.iter()
            .map(|child| self.calculate_section_word_count(child))
            .sum::<usize>();
    ...
    fn get_section_by_index(&self, index: usize) -> Option<&StructuredSection> {
        if index < 1000 {
            ...
            self.toc.get(parent_index)?.children.get(child_index)
        }
    }
    ...
    fn find_section_recursive_mut<'a>(
        ...
        for section in sections {
            ...
            if let Some(found) = Self::find_section_recursive_mut(&mut section.children, target_id) {
                return Some(found);
            }
        }
        None
    ...
    fn find_section_recursive<'a>(
        ...
        for section in sections {
            ...
            if let Some(found) = self.find_section_recursive(&section.children, target_id) {
                return Some(found);
            }
        }
        None
    ...
    fn collect_all_sections<'a>(
        ...
        for section in sections {
            result.push(section);
            self.collect_all_sections(&section.children, result);
        }
    }

    /// 更新章节内容
    pub fn update_section_content(&mut self, section_id: &str, new_content: String) -> Result<bool, AppError> {
    ...
    fn remove_section_recursive(sections: &mut Vec<StructuredSection>, target_id: &str) -> bool {
        for i in 0..sections.len() {
            ...
            if Self::remove_section_recursive(&mut sections[i].children, target_id) {
                return true;
            }
        }
        false
    ...
    pub fn get_statistics(&mut self) -> DocumentStatistics {
        ...
        let large_sections = all_sections.iter()
            .filter(|s| s.is_large_content())
            .count();
        
        let max_depth = self.toc.iter()
            .map(|s| s.get_depth())
            .max()
            .unwrap_or(0);
    ...
    /// 递归清理章节
    fn cleanup_sections(sections: &mut Vec<StructuredSection>, removed_count: &mut usize) -> Result<(), AppError> {
        let mut i = 0;
        while i < sections.len() {
            ...
            Self::cleanup_sections(&mut section.children, removed_count)?;
    ...
    pub fn get_memory_usage(&self) -> MemoryUsage {
        ...
        let index_size = self.section_index.len() * (std::mem::size_of::<String>() + std::mem::size_of::<usize>())
            + self.level_index.len() * (std::mem::size_of::<u8>() + std::mem::size_of::<Vec<usize>>());
    ...
    /// 递归计算内存使用
    fn calculate_memory_usage(&self, sections: &[StructuredSection], content_size: &mut usize, count: &mut usize) {
        for section in sections {
            *content_size += section.content.len();
            *count += 1;
            self.calculate_memory_usage(&section.children, content_size, count);
        }
    }
}
...
mod tests {
    ...
    fn test_structured_document_update_section_content() {
        ...
        let changed = doc.update_section_content("section1", "New content".to_string()).unwrap();
        assert!(changed);
        ...
        assert!(doc.update_section_content("nonexistent", "Content".to_string()).is_err());
    }
