...
mod task_scheduler_tests {
    use crate::cluster::{ClusterState, task_scheduler::{SimpleTaskScheduler, SchedulerConfig}};
    use crate::models::{ClusterNode, NodeRole, NodeStatus, TaskState, ClusterError};
    use std::sync::Arc;
    ...
    async fn test_round_robin_assignment_followers_only() {
        ...
        cluster_state.upsert_node(create_test_node("leader", NodeRole::Leader, NodeStatus::Healthy));
        ...
        for (node_id, count) in node_counts {
            ...
            assert_eq!(count, 2, "Node {} should have exactly 2 tasks, got {}", node_id, count);
        }
    }
    ...
    async fn test_round_robin_assignment_with_leader() {
        ...
        assert!(node_counts.contains_key("leader"), "Leader should process tasks");
        
        let follower_nodes: Vec<_> = node_counts.keys()
    ...
    async fn test_no_available_nodes() {
        ...
        let task_id = cluster_state.create_task("client-1".to_string(), "test.wav".to_string(), None, None);
    ...
    async fn test_only_unhealthy_nodes() {
        ...
        // Add unhealthy nodes
        cluster_state.upsert_node(create_test_node("node-1", NodeRole::Follower, NodeStatus::Unhealthy));
        cluster_state.upsert_node(create_test_node("node-2", NodeRole::Follower, NodeStatus::Joining));
    ...
    async fn test_node_capacity_limits() {
        ...
        // Add one node
        cluster_state.upsert_node(create_test_node("worker", NodeRole::Follower, NodeStatus::Healthy));
        ...
        let overflow_task = cluster_state.create_task("client-overflow".to_string(), "overflow.wav".to_string(), None, None);
        ...
        let new_task = cluster_state.create_task("client-new".to_string(), "new.wav".to_string(), None, None);
    ...
    async fn test_mixed_node_statuses() {
        ...
        cluster_state.upsert_node(create_test_node("healthy-1", NodeRole::Follower, NodeStatus::Healthy));
        cluster_state.upsert_node(create_test_node("unhealthy", NodeRole::Follower, NodeStatus::Unhealthy));
        cluster_state.upsert_node(create_test_node("healthy-2", NodeRole::Follower, NodeStatus::Healthy));
        cluster_state.upsert_node(create_test_node("joining", NodeRole::Follower, NodeStatus::Joining));
        ...
        for node in &assigned_nodes {
            assert!(node == "healthy-1" || node == "healthy-2", "Task assigned to unhealthy node: {}", node);
    ...
    async fn test_node_becomes_unhealthy_during_assignment() {
        ...
        // Add healthy nodes
        cluster_state.upsert_node(create_test_node("node-1", NodeRole::Follower, NodeStatus::Healthy));
        cluster_state.upsert_node(create_test_node("node-2", NodeRole::Follower, NodeStatus::Healthy));
        
        let scheduler = create_test_scheduler(cluster_state.clone(), false, "leader");
        
        // Assign first task - should go to one of the healthy nodes
        let task1 = cluster_state.create_task("client-1".to_string(), "test1.wav".to_string(), None, None);
        let assigned1 = scheduler.assign_next_task(task1).await.unwrap();
        assert!(assigned1 == "node-1" || assigned1 == "node-2", "Task assigned to unexpected node: {}", assigned1);
        
        // Make one node unhealthy (the other one should still be healthy)
        let unhealthy_node = if assigned1 == "node-1" { "node-1" } else { "node-2" };
        let healthy_node = if assigned1 == "node-1" { "node-2" } else { "node-1" };
        cluster_state.update_node_status(unhealthy_node, NodeStatus::Unhealthy).unwrap();
        
        // Assign second task - should go to the remaining healthy node
        let task2 = cluster_state.create_task("client-2".to_string(), "test2.wav".to_string(), None, None);
        ...
        let task3 = cluster_state.create_task("client-3".to_string(), "test3.wav".to_string(), None, None);
    ...
    async fn test_should_leader_process_logic() {
        ...
        let scheduler_with_leader = create_test_scheduler(cluster_state.clone(), true, "leader-node");
        ...
        let scheduler_no_leader = create_test_scheduler(cluster_state.clone(), false, "leader-node");
    ...
    async fn test_task_assignment_with_existing_tasks() {
        ...
        let existing_task1 = cluster_state.create_task("existing-1".to_string(), "existing1.wav".to_string(), None, None);
        let existing_task2 = cluster_state.create_task("existing-2".to_string(), "existing2.wav".to_string(), None, None);
        
        cluster_state.assign_task(&existing_task1, "node-1").unwrap();
        cluster_state.assign_task(&existing_task2, "node-1").unwrap();
        ...
        let new_task = cluster_state.create_task("new-client".to_string(), "new.wav".to_string(), None, None);
        let assigned_node = scheduler.assign_next_task(new_task).await.unwrap();
    ...
    async fn test_concurrent_task_assignments() {
        ...
        let scheduler = Arc::new(create_test_scheduler(cluster_state.clone(), false, "leader"));
    ...
    async fn test_node_addition_during_operation() {
        ...
        let task2 = cluster_state.create_task("client-2".to_string(), "test2.wav".to_string(), None, None);
        let assigned2 = scheduler.assign_next_task(task2).await.unwrap();
        assert!(assigned2 == "node-1" || assigned2 == "node-2", "Task assigned to unexpected node: {}", assigned2);
        
        // Assign third task - should go to the other node (round-robin)
        let task3 = cluster_state.create_task("client-3".to_string(), "test3.wav".to_string(), None, None);
        let assigned3 = scheduler.assign_next_task(task3).await.unwrap();
        assert!(assigned3 == "node-1" || assigned3 == "node-2", "Task assigned to unexpected node: {}", assigned3);
        ...
        assert!(node1_tasks > 0 && node2_tasks > 0, "Both nodes should have tasks");
    }
    ...
    async fn test_cluster_state_integration() {
        ...
        scheduler.update_node_status("node-1", NodeStatus::Unhealthy).unwrap();
