...
mod cluster_state_tests {
    ...
    use crate::models::{ClusterNode, TaskMetadata, NodeRole, NodeStatus, TaskState, ClusterError};
    use std::sync::Arc;
    ...
    fn create_test_task(task_id: &str, client_id: &str, filename: &str) -> TaskMetadata {
        TaskMetadata::new(task_id.to_string(), client_id.to_string(), filename.to_string())
    }
    ...
    fn test_node_status_update() {
        ...
        // Update status
        state.update_node_status("node-1", NodeStatus::Healthy).unwrap();
    ...
    fn test_nodes_by_role_and_status() {
        ...
        state.upsert_node(create_test_node("leader-1", NodeRole::Leader, NodeStatus::Healthy));
        state.upsert_node(create_test_node("follower-1", NodeRole::Follower, NodeStatus::Healthy));
        state.upsert_node(create_test_node("follower-2", NodeRole::Follower, NodeStatus::Unhealthy));
        state.upsert_node(create_test_node("candidate-1", NodeRole::Candidate, NodeStatus::Joining));
    ...
    fn test_task_operations() {
        ...
        // Test failing task
        let task_id2 = state.create_task("client-2".to_string(), "test2.wav".to_string(), None, None);
    ...
    fn test_task_reassignment() {
        ...
        // Add two nodes
        state.upsert_node(create_test_node("node-1", NodeRole::Follower, NodeStatus::Healthy));
        state.upsert_node(create_test_node("node-2", NodeRole::Follower, NodeStatus::Healthy));
    ...
    fn test_tasks_by_state() {
        ...
        // Update states
        state.update_task_state(&task2, TaskState::Processing).unwrap();
    ...
    fn test_cluster_stats() {
        ...
        // Add nodes
        state.upsert_node(create_test_node("leader", NodeRole::Leader, NodeStatus::Healthy));
        state.upsert_node(create_test_node("follower-1", NodeRole::Follower, NodeStatus::Healthy));
        state.upsert_node(create_test_node("follower-2", NodeRole::Follower, NodeStatus::Unhealthy));
    ...
    async fn test_concurrent_task_operations() {
        ...
        // Add a node first
        state.upsert_node(create_test_node("worker", NodeRole::Follower, NodeStatus::Healthy));
    ...
    async fn test_concurrent_status_updates() {
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                ...
                for status in [NodeStatus::Healthy, NodeStatus::Unhealthy, NodeStatus::Healthy] {
                    state_clone.update_node_status(&format!("node-{}", i), status).unwrap();
