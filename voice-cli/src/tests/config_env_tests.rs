...
mod config_env_tests {
    ...
    fn test_port_conflict_validation() {
        ...
        assert!(result.is_err());
        let error_msg = result.unwrap_err().to_string();
        assert!(error_msg.contains("gRPC port and HTTP port cannot be the same"),
                "Expected port conflict error, got: {}", error_msg);
...
mod config_hot_reload_tests {
    ...
    async fn test_config_change_notifications() {
        ...
        let notification = tokio::time::timeout(Duration::from_secs(1), change_receiver.recv()).await;
    ...
    async fn test_hot_reload_watcher() {
        ...
        let notification = tokio::time::timeout(Duration::from_secs(3), change_receiver.recv()).await;
        assert!(notification.is_ok());
