...
impl Config {
    ...
    pub fn load_with_env_overrides(config_path: &PathBuf) -> crate::Result<Self> {
        let mut config = if config_path.exists() {
            let config_content = std::fs::read_to_string(config_path)
                .map_err(|e| crate::VoiceCliError::Config(
                    format!("Failed to read configuration file {:?}: {}", config_path, e)
                ))?;
            
            serde_yaml::from_str(&config_content)
                .map_err(|e| crate::VoiceCliError::Config(
                    format!("Failed to parse configuration file {:?}: {}", config_path, e)
                ))?
        } else {
            let default_config = Config::default();
            default_config.save(config_path)
                .map_err(|e| crate::VoiceCliError::Config(
                    format!("Failed to create default configuration file {:?}: {}", config_path, e)
                ))?;
    ...
    pub fn apply_env_overrides(&mut self) -> crate::Result<()> {
        ...
        if let Ok(port_str) = std::env::var("VOICE_CLI_GRPC_PORT") {
            let port = port_str.parse::<u16>()
                .map_err(|_| crate::VoiceCliError::Config(
                    format!("Invalid VOICE_CLI_GRPC_PORT value '{}': must be a valid port number (1-65535)", port_str)
                ))?;
            self.cluster.grpc_port = port;
            tracing::info!("Applied environment override: VOICE_CLI_GRPC_PORT = {}", port);
        }
        ...
        if let Ok(size_str) = std::env::var("VOICE_CLI_MAX_FILE_SIZE") {
            let size = size_str.parse::<usize>()
                .map_err(|_| crate::VoiceCliError::Config(
                    format!("Invalid VOICE_CLI_MAX_FILE_SIZE value '{}': must be a valid number in bytes", size_str)
                ))?;
            if size == 0 {
                return Err(crate::VoiceCliError::Config(
            ...
            tracing::info!("Applied environment override: VOICE_CLI_MAX_FILE_SIZE = {}", size);
        }

        // CORS enabled override
        if let Ok(cors_str) = std::env::var("VOICE_CLI_CORS_ENABLED") {
            let cors_enabled = cors_str.parse::<bool>()
                .map_err(|_| crate::VoiceCliError::Config(
                    format!("Invalid VOICE_CLI_CORS_ENABLED value '{}': must be 'true' or 'false'", cors_str)
                ))?;
            self.server.cors_enabled = cors_enabled;
            tracing::info!("Applied environment override: VOICE_CLI_CORS_ENABLED = {}", cors_enabled);
        }
        ...
        if let Ok(node_id) = std::env::var("VOICE_CLI_NODE_ID") {
            ...
            tracing::info!("Applied environment override: VOICE_CLI_NODE_ID = {}", node_id);
        }

        if let Ok(enabled_str) = std::env::var("VOICE_CLI_CLUSTER_ENABLED") {
            let enabled = enabled_str.parse::<bool>()
                .map_err(|_| crate::VoiceCliError::Config(
                    format!("Invalid VOICE_CLI_CLUSTER_ENABLED value '{}': must be 'true' or 'false'", enabled_str)
                ))?;
            self.cluster.enabled = enabled;
            tracing::info!("Applied environment override: VOICE_CLI_CLUSTER_ENABLED = {}", enabled);
        }
        ...
        if let Ok(bind_address) = std::env::var("VOICE_CLI_BIND_ADDRESS") {
            ...
            tracing::info!("Applied environment override: VOICE_CLI_BIND_ADDRESS = {}", bind_address);
        }
        ...
        if let Ok(can_process_str) = std::env::var("VOICE_CLI_LEADER_CAN_PROCESS_TASKS") {
            ...
            tracing::info!("Applied environment override: VOICE_CLI_LEADER_CAN_PROCESS_TASKS = {}", can_process);
        }
        ...
        if let Ok(interval_str) = std::env::var("VOICE_CLI_HEARTBEAT_INTERVAL") {
            ...
            tracing::info!("Applied environment override: VOICE_CLI_HEARTBEAT_INTERVAL = {}", interval);
        }
        ...
        if let Ok(timeout_str) = std::env::var("VOICE_CLI_ELECTION_TIMEOUT") {
            ...
            tracing::info!("Applied environment override: VOICE_CLI_ELECTION_TIMEOUT = {}", timeout);
        }
        ...
        if let Ok(enabled_str) = std::env::var("VOICE_CLI_LB_ENABLED") {
            let enabled = enabled_str.parse::<bool>()
                .map_err(|_| crate::VoiceCliError::Config(
                    format!("Invalid VOICE_CLI_LB_ENABLED value '{}': must be 'true' or 'false'", enabled_str)
                ))?;
            self.load_balancer.enabled = enabled;
            tracing::info!("Applied environment override: VOICE_CLI_LB_ENABLED = {}", enabled);
        }

        if let Ok(port_str) = std::env::var("VOICE_CLI_LB_PORT") {
            let port = port_str.parse::<u16>()
                .map_err(|_| crate::VoiceCliError::Config(
                    format!("Invalid VOICE_CLI_LB_PORT value '{}': must be a valid port number (1-65535)", port_str)
                ))?;
        ...
        if let Ok(bind_address) = std::env::var("VOICE_CLI_LB_BIND_ADDRESS") {
            ...
            tracing::info!("Applied environment override: VOICE_CLI_LB_BIND_ADDRESS = {}", bind_address);
        }
        ...
        if let Ok(interval_str) = std::env::var("VOICE_CLI_LB_HEALTH_CHECK_INTERVAL") {
            ...
            tracing::info!("Applied environment override: VOICE_CLI_LB_HEALTH_CHECK_INTERVAL = {}", interval);
        }
        ...
        if let Ok(timeout_str) = std::env::var("VOICE_CLI_LB_HEALTH_CHECK_TIMEOUT") {
            ...
            tracing::info!("Applied environment override: VOICE_CLI_LB_HEALTH_CHECK_TIMEOUT = {}", timeout);
        }
        ...
        if let Ok(level) = std::env::var("VOICE_CLI_LOG_LEVEL") {
            ...
            if !valid_levels.contains(&level.as_str()) {
                return Err(crate::VoiceCliError::Config(
                    format!("Invalid VOICE_CLI_LOG_LEVEL value '{}': must be one of {:?}", level, valid_levels)
                ));
            }
            self.logging.level = level.clone();
            tracing::info!("Applied environment override: VOICE_CLI_LOG_LEVEL = {}", level);
        }
        ...
        if let Ok(log_dir) = std::env::var("VOICE_CLI_LOG_DIR") {
            ...
            tracing::info!("Applied environment override: VOICE_CLI_LOG_DIR = {}", log_dir);
        }

        if let Ok(max_files_str) = std::env::var("VOICE_CLI_LOG_MAX_FILES") {
            let max_files = max_files_str.parse::<u32>()
                .map_err(|_| crate::VoiceCliError::Config(
                    format!("Invalid VOICE_CLI_LOG_MAX_FILES value '{}': must be a valid number", max_files_str)
                ))?;
            if max_files == 0 {
                return Err(crate::VoiceCliError::Config(
            ...
            tracing::info!("Applied environment override: VOICE_CLI_LOG_MAX_FILES = {}", max_files);
        }
        ...
        if let Ok(model) = std::env::var("VOICE_CLI_DEFAULT_MODEL") {
            ...
            tracing::info!("Applied environment override: VOICE_CLI_DEFAULT_MODEL = {}", model);
        }
        ...
        if let Ok(models_dir) = std::env::var("VOICE_CLI_MODELS_DIR") {
            ...
            tracing::info!("Applied environment override: VOICE_CLI_MODELS_DIR = {}", models_dir);
        }

        if let Ok(auto_download_str) = std::env::var("VOICE_CLI_AUTO_DOWNLOAD") {
            let auto_download = auto_download_str.parse::<bool>()
                .map_err(|_| crate::VoiceCliError::Config(
                    format!("Invalid VOICE_CLI_AUTO_DOWNLOAD value '{}': must be 'true' or 'false'", auto_download_str)
                ))?;
            self.whisper.auto_download = auto_download;
            tracing::info!("Applied environment override: VOICE_CLI_AUTO_DOWNLOAD = {}", auto_download);
        }

        if let Ok(workers_str) = std::env::var("VOICE_CLI_TRANSCRIPTION_WORKERS") {
            let workers = workers_str.parse::<usize>()
                .map_err(|_| crate::VoiceCliError::Config(
                    format!("Invalid VOICE_CLI_TRANSCRIPTION_WORKERS value '{}': must be a valid number", workers_str)
                ))?;
            if workers == 0 {
                return Err(crate::VoiceCliError::Config(
            ...
            tracing::info!("Applied environment override: VOICE_CLI_TRANSCRIPTION_WORKERS = {}", workers);
        }
        ...
        if let Ok(db_path) = std::env::var("VOICE_CLI_METADATA_DB_PATH") {
            ...
            tracing::info!("Applied environment override: VOICE_CLI_METADATA_DB_PATH = {}", db_path);
        }
        ...
        if let Ok(work_dir) = std::env::var("VOICE_CLI_WORK_DIR") {
            ...
            tracing::info!("Applied environment override: VOICE_CLI_WORK_DIR = {}", work_dir);
        }
        ...
        if let Ok(pid_file) = std::env::var("VOICE_CLI_PID_FILE") {
            ...
            tracing::info!("Applied environment override: VOICE_CLI_PID_FILE = {}", pid_file);
        }
    ...
    pub fn validate(&self) -> crate::Result<()> {
        ...
        if self.server.port == 0 {
            return Err(crate::VoiceCliError::Config("Server port must be between 1 and 65535".to_string()));
        }
        ...
        if self.server.max_file_size == 0 {
            return Err(crate::VoiceCliError::Config("Max file size must be greater than 0".to_string()));
        }
        ...
        if self.whisper.models_dir.is_empty() {
            return Err(crate::VoiceCliError::Config("Models directory cannot be empty".to_string()));
        }

        if self.whisper.workers.transcription_workers == 0 {
            return Err(crate::VoiceCliError::Config("Transcription workers must be greater than 0".to_string()));
        }

        // Validate logging configuration
        if self.logging.log_dir.is_empty() {
            return Err(crate::VoiceCliError::Config("Log directory cannot be empty".to_string()));
        }
        ...
        if !valid_log_levels.contains(&self.logging.level.to_lowercase().as_str()) {
            return Err(crate::VoiceCliError::Config(
                format!("Invalid log level '{}'. Valid levels: {:?}", self.logging.level, valid_log_levels)
            ));
        }

        // Validate daemon configuration
        if self.daemon.work_dir.is_empty() {
            return Err(crate::VoiceCliError::Config("Work directory cannot be empty".to_string()));
        }
        
        if self.daemon.pid_file.is_empty() {
            return Err(crate::VoiceCliError::Config("PID file path cannot be empty".to_string()));
        }

        // Validate cluster configuration if enabled
        if self.cluster.enabled {
            if self.cluster.grpc_port == 0 {
                return Err(crate::VoiceCliError::Config("gRPC port must be between 1 and 65535".to_string()));
            }

            // Note: u16 max value is 65535, so no need to check upper bound

            if self.cluster.http_port == 0 {
                return Err(crate::VoiceCliError::Config("Invalid cluster HTTP port: must be > 0".to_string()));
            }

            // Note: u16 max value is 65535, so no need to check upper bound
            if self.cluster.heartbeat_interval == 0 {
                return Err(crate::VoiceCliError::Config("Heartbeat interval must be greater than 0".to_string()));
            }
            
            if self.cluster.election_timeout == 0 {
                return Err(crate::VoiceCliError::Config("Election timeout must be greater than 0".to_string()));
            }
            ...
            if self.cluster.node_id.trim().is_empty() {
                return Err(crate::VoiceCliError::Config("Node ID cannot be empty when cluster is enabled".to_string()));
            }

            if self.cluster.bind_address.is_empty() {
                return Err(crate::VoiceCliError::Config("Cluster bind address cannot be empty".to_string()));
            }

            if self.cluster.metadata_db_path.is_empty() {
                return Err(crate::VoiceCliError::Config("Metadata database path cannot be empty".to_string()));
            }
            ...
            if self.cluster.http_port == self.server.port {
                tracing::warn!("Cluster HTTP port is the same as server port: {}", self.server.port);
            }
        }
        ...
        if self.load_balancer.enabled {
            if self.load_balancer.port == 0 {
                return Err(crate::VoiceCliError::Config("Load balancer port must be between 1 and 65535".to_string()));
            }

            // Note: u16 max value is 65535, so this check is redundant but kept for clarity

            if self.load_balancer.health_check_interval == 0 {
                return Err(crate::VoiceCliError::Config("Health check interval must be greater than 0".to_string()));
            }

            if self.load_balancer.health_check_timeout == 0 {
                return Err(crate::VoiceCliError::Config("Health check timeout must be greater than 0".to_string()));
            }
            ...
            if self.load_balancer.bind_address.is_empty() {
                return Err(crate::VoiceCliError::Config("Load balancer bind address cannot be empty".to_string()));
            }
