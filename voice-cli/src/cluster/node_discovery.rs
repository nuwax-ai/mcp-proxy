...
impl NodeDiscovery {
    ...
    pub async fn start(&self) -> Result<(), ClusterError> {
        info!("Starting node discovery service for node {}", self.local_node.node_id);
    ...
    async fn discover_peers(&self) -> Result<(), ClusterError> {
        ...
        for bootstrap_addr in &self.config.bootstrap_peers {
            if let Err(e) = self.discover_from_bootstrap(bootstrap_addr).await {
                debug!("Failed to discover from bootstrap {}: {}", bootstrap_addr, e);
            }
        }
    ...
    async fn discover_from_bootstrap(&self, bootstrap_addr: &str) -> Result<(), ClusterError> {
        ...
        if parts.len() != 2 {
            return Err(ClusterError::Config(format!("Invalid bootstrap address: {}", bootstrap_addr)));
        }

        let addr = parts[0].to_string();
        let grpc_port: u16 = parts[1].parse()
            .map_err(|_| ClusterError::Config(format!("Invalid port in bootstrap address: {}", bootstrap_addr)))?;
        ...
        match timeout(self.config.health_check_timeout, self.client.get(&health_url).send()).await {
            ...
            Ok(Ok(response)) => {
                debug!("Bootstrap node {} returned status: {}", bootstrap_addr, response.status());
            }
            Ok(Err(e)) => {
                debug!("Failed to connect to bootstrap node {}: {}", bootstrap_addr, e);
            }
            Err(_) => {
        ...
        match timeout(
            ...
            Ok(Ok(response)) if response.status().is_success() => {
                ...
                if let Ok(text) = response.text().await {
                    ...
                    debug!("Got cluster status from peer {}: {} bytes", peer.node_id, text.len());
                }
            }
            Ok(Ok(response)) => {
                debug!("Peer {} returned status: {}", peer.node_id, response.status());
            }
            Ok(Err(e)) => {
    ...
    async fn check_peer_health(&self) -> Result<(), ClusterError> {
        ...
        for (node_id, peer) in peers {
            if let Err(e) = self.check_single_peer_health(&peer).await {
                ...
                if let Err(e) = self.metadata_store.update_node_status(&node_id, NodeStatus::Unhealthy).await {
                    warn!("Failed to update peer {} status to unhealthy: {}", node_id, e);
                }
            } else {
                ...
                if peer.status == NodeStatus::Unhealthy {
                    if let Err(e) = self.metadata_store.update_node_status(&node_id, NodeStatus::Healthy).await {
    ...
    pub async fn join_cluster(&self, peer_address: &str) -> Result<(), ClusterError> {
        ...
        if parts.len() != 2 {
            return Err(ClusterError::Config(format!("Invalid peer address: {}", peer_address)));
        }

        let addr = parts[0].to_string();
        let grpc_port: u16 = parts[1].parse()
            .map_err(|_| ClusterError::Config(format!("Invalid port in peer address: {}", peer_address)))?;
    ...
    pub async fn leave_cluster(&self) -> Result<(), ClusterError> {
        ...
        self.metadata_store.update_node_status(&self.local_node.node_id, NodeStatus::Leaving).await?;
        ...
        self.metadata_store.remove_node(&self.local_node.node_id).await?;
