...
impl TranscriptionWorkerPool {
    ...
    pub async fn new(config: Arc<Config>) -> Result<Self, VoiceCliError> {
        ...
        info!(
            "Initializing transcription worker pool with {} workers",
            worker_count
        );
    ...
    pub async fn submit_task(&self, task: TranscriptionTask) -> Result<(), VoiceCliError> {
        self.task_sender.send(task).await.map_err(|_| {
            VoiceCliError::WorkerPoolError("Failed to submit task to worker pool".to_string())
        })
    }
...
impl TranscriptionWorker {
    ...
    pub async fn run(
        &self,
        task_receiver: Arc<tokio::sync::Mutex<mpsc::Receiver<TranscriptionTask>>>,
    ) {
    ...
    async fn process_transcription_task(
        ...
        let processed_audio = self.process_audio_format(audio_data, &filename).await?;

        // 2. Perform transcription using voice-toolkit
        let transcription_result = self
            .perform_transcription(&processed_audio, &model, &response_format)
            .await?;
    ...
    async fn convert_to_whisper_format(&self, input_path: &Path) -> Result<PathBuf, VoiceCliError> {
