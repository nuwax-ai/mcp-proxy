...
impl AudioFormatDetector {
    ...
    pub fn detect_format(
        ...
        info!("Starting audio format detection for {} byte audio data", audio_data.len());
        
        // Try Symphonia probe first (primary method)
        if let Ok(result) = Self::symphonia_probe(audio_data, filename) {
            info!("Successfully detected format using Symphonia probe: {:?}", result.format);
            return Ok(result);
        }
    ...
    fn extract_format_info(probe_result: &ProbeResult) -> Result<AudioFormatResult, VoiceCliError> {
        ...
        let confidence = if format != AudioFormat::Unknown { 0.95 } else { 0.0 };
        
        if format == AudioFormat::Unknown {
            return Err(VoiceCliError::UnsupportedFormat(
                format!("Unsupported codec type: {:?}", track.codec_params.codec)
            ));
        }
    ...
    pub fn validate_format_support(format_result: &AudioFormatResult) -> Result<(), VoiceCliError> {
        if !format_result.format.is_supported() {
            return Err(VoiceCliError::UnsupportedFormat(
                format!("Format {} is not supported for transcription", format_result.format.to_string())
            ));
        }
        
        // Check confidence threshold
        if format_result.confidence < 0.3 {
            warn!("Low confidence format detection: {}", format_result.confidence);
        }
