use crate::grpc::proto::audio_cluster_service_server::{AudioClusterService, AudioClusterServiceServer};
use crate::grpc::proto::{
    JoinRequest, JoinResponse, LeaveRequest, LeaveResponse, ClusterStatusRequest, ClusterStatusResponse,
    HeartbeatRequest, HeartbeatResponse, TaskAssignmentRequest, TaskAssignmentResponse,
    TaskCompletionRequest, TaskCompletionResponse, NodeInfo, NodeRole, NodeStatus,
    TaskState, ClusterHealth,
};
use crate::models::{
...
impl AudioClusterServiceImpl {
    ...
    fn proto_to_cluster_node(&self, proto: &NodeInfo) -> Result<ClusterNode, ClusterError> {
        let role = match NodeRole::try_from(proto.role) {
            ...
            Err(_) => return Err(ClusterError::InvalidOperation("Invalid node role".to_string())),
        };
        ...
        let status = match NodeStatus::try_from(proto.status) {
            ...
            Err(_) => return Err(ClusterError::InvalidOperation("Invalid node status".to_string())),
        };
...
impl AudioClusterService for AudioClusterServiceImpl {
    ...
    async fn join_cluster(
        ...
        match self.metadata_store.add_node(&joining_node).await {
            Ok(_) => {
                info!("Successfully added node {} to cluster", joining_node.node_id);
    ...
    async fn get_cluster_status(
        ...
        let proto_nodes: Vec<NodeInfo> = nodes.iter()
            .map(|node| self.cluster_node_to_proto(node))
            .collect();

        // Find leader node
        let leader_node_id = nodes.iter()
    ...
    async fn heartbeat(
        ...
        // Update node status
        if let Err(e) = self.metadata_store.update_node_status(&req.node_id, status).await {
    ...
    async fn assign_task(
        ...
        // Get task scheduler
        let scheduler = self.task_scheduler.as_ref().ok_or_else(|| {
            Status::new(Code::Internal, "Task scheduler not available")
        })?;
        ...
        let task = TaskMetadata {
            ...
            completed_at: None,
            error_message: None,
            model: if req.model.is_empty() { None } else { Some(req.model.clone()) },
            response_format: if req.response_format.is_empty() { None } else { Some(req.response_format.clone()) },
    ...
    async fn report_task_completion(
        ...
        info!("Received task completion report: task_id={}, state={:?}", 
