...
pub async fn handle_model_download(config: &Config, model_name: &str) -> crate::Result<()> {
    ...
    // Validate model name
    if !config.whisper.supported_models.contains(&model_name.to_string()) {
        return Err(VoiceCliError::InvalidModelName(
            format!("Model '{}' is not supported. Supported models: {:?}", 
...
pub async fn handle_model_list(config: &Config) -> crate::Result<()> {
    ...
    for model in &config.whisper.supported_models {
        let is_downloaded = model_service.is_model_downloaded(model).await.unwrap_or(false);
        let status = if is_downloaded { "✓ Downloaded" } else { "○ Not Downloaded" };
        let default_marker = if model == &config.whisper.default_model { " (default)" } else { "" };
    ...
    if downloaded_models.is_empty() {
        ...
        for model_name in downloaded_models {
            match model_service.get_model_info(&model_name).await {
                Ok(info) => {
                    println!("  {} - Size: {}, Status: {}", model_name, info.size, info.status);
                }
                Err(e) => {
...
pub async fn ensure_default_model(config: &Config) -> crate::Result<()> {
    ...
    if model_service.is_model_downloaded(&config.whisper.default_model).await? {
        return Ok(());
    }
    ...
    if config.whisper.auto_download {
        info!("No models found. Auto-downloading default model: {}", config.whisper.default_model);
