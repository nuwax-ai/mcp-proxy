...
mod config_template_tests {
    ...
    fn test_config_template_inclusion() {
        ...
        assert!(config_template.contains("load_balancer:"), "应该包含load_balancer配置");
        assert!(config_template.contains("leader_can_process_tasks:"), "应该包含leader_can_process_tasks配置");
        assert!(config_template.contains("grpc_port:"), "应该包含grpc_port配置");
        assert!(config_template.contains("health_check_interval:"), "应该包含health_check_interval配置");
        
        println!("✅ 配置模板包含所有必要的配置项，长度: {} 字节", config_template.len());
    }
    ...
    fn test_generate_config_with_template() {
        ...
        // 读取生成的配置文件内容
        let generated_content = std::fs::read_to_string(&config_path)
            .expect("读取生成的配置文件失败");
        
        // 验证生成的内容包含关键配置项
        assert!(generated_content.contains("cluster:"), "生成的配置应该包含cluster配置");
        assert!(generated_content.contains("load_balancer:"), "生成的配置应该包含load_balancer配置");
        assert!(generated_content.contains("leader_can_process_tasks: true"), "生成的配置应该包含leader_can_process_tasks配置");
        
        println!("✅ 配置文件生成成功，内容长度: {} 字节", generated_content.len());
    }
    ...
    fn test_template_yaml_validity() {
        ...
        match yaml_result {
            Ok(yaml_value) => {
                ...
                assert!(yaml_value.get("load_balancer").is_some(), "应该有load_balancer配置节点");
            }
            Err(e) => {
