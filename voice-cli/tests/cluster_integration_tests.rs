...
use voice_cli::{
    models::{ClusterNode, TaskMetadata, MetadataStore, NodeRole, TaskState, Config},
    cluster::{SimpleTaskScheduler, SimpleTranscriptionWorker, SchedulerConfig, WorkerConfig},
...
async fn test_cluster_node_lifecycle() {
    ...
    metadata_store.create_task(&TaskMetadata::new(
...
async fn test_metadata_store_concurrency() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let retrieved = store.get_task(&format!("concurrent-task-{}", i)).await.unwrap();
    ...
    for i in 0..10 {
        let task = metadata_store.get_task(&format!("concurrent-task-{}", i)).await.unwrap();
        assert!(task.is_some());
    }
