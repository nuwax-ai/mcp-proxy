...
use voice_cli::{
    cluster::transcription_worker::{
        SimpleTranscriptionWorker, TaskAssignmentRequest, WorkerConfig, WorkerEvent,
    },
    models::MetadataStore,
    models::{ClusterError, ClusterTranscriptionResult, Config},
    server::handlers::AppState,
    services::{ModelService, TranscriptionWorkerPool},
};
...
async fn test_real_stt_engine_with_cluster_worker() {
    ...
    match result {
        Ok(transcription_result) => {
            ...
            println!(
                "✅ Real STT transcription successful: {}",
            ...
            Ok(Ok(result)) => {
                if result.success {
                    if let Some(response) = result.response {
                        println!(
                            "✅ Format {} processed successfully: {} chars",
                            filename,
                            response.text.len()
                        );
...
async fn test_stt_engine_error_handling() {
    ...
    assert!(
        result.is_err(),
        "Expected error for non-existent audio file"
    );
...
async fn test_stt_http_server_integration() {
    ...
    assert!(
        ...
        "Should have supported models configured"
    );

    let loaded_models = app_state.model_service.list_loaded_models().await;
    assert!(
        loaded_models.is_ok(),
        "Should be able to list loaded models"
    );
...
async fn test_concurrent_stt_processing() {
    ...
    println!(
        "✅ Concurrent processing test completed: {}/{} successful",
        successful_tasks,
        test_files.len()
    );
}
...
async fn create_test_audio_file() -> NamedTempFile {
    ...
    tokio::fs::write(temp_file.path(), wav_data)
        .await
