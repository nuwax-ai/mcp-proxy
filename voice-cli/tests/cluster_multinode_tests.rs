...
use voice_cli::{
    models::{ClusterNode, TaskMetadata, MetadataStore, NodeRole, NodeStatus, TaskState},
    cluster::{SimpleTaskScheduler, SchedulerConfig},
};
use std::sync::Arc;
...
async fn test_multi_node_task_distribution() {
    ...
    metadata_store.add_node(&leader_node_with_role).await.unwrap();
    ...
    let completed_tasks = metadata_store.get_tasks_by_state(TaskState::Completed).await.unwrap();
    let assigned_tasks = metadata_store.get_tasks_by_state(TaskState::Assigned).await.unwrap();
    let pending_tasks = metadata_store.get_tasks_by_state(TaskState::Pending).await.unwrap();
    ...
    let mut worker_task_counts: std::collections::HashMap<String, i32> = std::collections::HashMap::new();
    ...
    assert!(worker_task_counts.values().all(|&count| count >= 1 && count <= 2));
...
async fn test_cluster_statistics_and_monitoring() {
    ...
    let task_scenarios = vec![
        ("completed-1", "client-1", "audio1.mp3", TaskState::Completed, Some("stats-worker-1")),
        ("completed-2", "client-2", "audio2.mp3", TaskState::Completed, Some("stats-worker-2")),
        ("processing-1", "client-1", "audio3.mp3", TaskState::Processing, Some("stats-worker-1")),
        ("assigned-1", "client-3", "audio4.mp3", TaskState::Assigned, Some("stats-worker-2")),
        ("failed-1", "client-2", "audio5.mp3", TaskState::Failed, Some("stats-worker-1")),
        ("pending-1", "client-1", "audio6.mp3", TaskState::Pending, None),
    ];
    ...
    for (node_id, stats) in &cluster_stats.node_stats {
        println!("Node {}: assigned={}, completed={}, failed={}", 
    ...
    assert_eq!(cluster_stats.leader_node_id, Some("stats-leader".to_string()));
...
async fn test_concurrent_multi_node_processing() {
    ...
    // Create cluster nodes
    let worker_nodes = vec!["concurrent-worker-1", "concurrent-worker-2", "concurrent-worker-3"];
    ...
    for i in 0..num_tasks {
        ...
        let handle = tokio::spawn(async move {
            ...
            store.complete_task(&task_id, processing_duration).await.unwrap();
