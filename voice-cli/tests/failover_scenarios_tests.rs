...
use voice_cli::{
    models::{ClusterNode, TaskMetadata, MetadataStore, NodeRole, NodeStatus, TaskState, LoadBalancerConfig},
    cluster::{SimpleTaskScheduler, SchedulerConfig, SchedulerEvent},
...
async fn test_failover_during_task_processing() {
    ...
    for (task_id, client_id, filename) in &tasks {
        ...
        metadata_store.assign_task(task_id, worker_node).await.unwrap();
    }
    
    sleep(Duration::from_millis(100)).await;
    
    // Verify tasks are assigned
    let assigned_tasks = metadata_store.get_tasks_by_state(TaskState::Assigned).await.unwrap();
    ...
    let worker1_tasks = metadata_store.get_tasks_by_node("task-worker-1").await.unwrap();
    ...
    metadata_store.assign_task("post-recovery-task", "task-worker-1").await.unwrap();
    
    // Verify tasks can be assigned to recovered worker
    let assigned_tasks = metadata_store.get_tasks_by_state(TaskState::Assigned).await.unwrap();
    let post_recovery_task = assigned_tasks.iter()
...
async fn test_leader_election_failover() {
    ...
    let nodes = vec![
        ("leader-original", NodeRole::Leader, NodeStatus::Healthy, 50070, 8110),
        ("candidate-1", NodeRole::Follower, NodeStatus::Healthy, 50071, 8111),
        ("candidate-2", NodeRole::Follower, NodeStatus::Healthy, 50072, 8112),
    ];
    ...
    let initial_leader = initial_nodes.iter()
    ...
    let new_leader = post_election_nodes.iter()
...
async fn test_network_partition_scenarios() {
    ...
    let nodes = vec![
        ("partition-leader", NodeRole::Leader, NodeStatus::Healthy, 50080, 8120),
        ("partition-node-1", NodeRole::Follower, NodeStatus::Healthy, 50081, 8121),
        ("partition-node-2", NodeRole::Follower, NodeStatus::Healthy, 50082, 8122),
        ("partition-node-3", NodeRole::Follower, NodeStatus::Healthy, 50083, 8123),
        ("partition-node-4", NodeRole::Follower, NodeStatus::Healthy, 50084, 8124),
    ];
    ...
    let healthy_nodes = metadata_store.get_all_nodes().await.unwrap()
        .into_iter()
    ...
    let final_healthy_nodes = metadata_store.get_all_nodes().await.unwrap()
        .into_iter()
        .filter(|n| n.status == NodeStatus::Healthy)
        .collect::<Vec<_>>();
...
async fn test_cascading_failure_recovery() {
    ...
    let nodes = vec![
        ("cascade-leader", NodeRole::Leader, NodeStatus::Healthy, 50090, 8130),
        ("cascade-worker-1", NodeRole::Follower, NodeStatus::Healthy, 50091, 8131),
        ("cascade-worker-2", NodeRole::Follower, NodeStatus::Healthy, 50092, 8132),
        ("cascade-worker-3", NodeRole::Follower, NodeStatus::Healthy, 50093, 8133),
    ];
    ...
    for i in 1..=6 {
        ...
        metadata_store.assign_task(&task.task_id, &worker_node).await.unwrap();
    }
    
    sleep(Duration::from_millis(100)).await;
    
    // Verify initial distribution
    let initial_assigned = metadata_store.get_tasks_by_state(TaskState::Assigned).await.unwrap();
    ...
    let remaining_workers = metadata_store.get_all_nodes().await.unwrap()
        .into_iter()
    ...
    // Verify recovery
    let final_healthy_workers = metadata_store.get_all_nodes().await.unwrap()
        .into_iter()
        .filter(|n| n.role == NodeRole::Follower && n.status == NodeStatus::Healthy)
        .count();
