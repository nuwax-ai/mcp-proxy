...
async fn test_cluster_state_metadata_store_integration() {
    ...
    metadata_store
        .update_node_status("test-node-1", NodeStatus::Healthy)
        .await
        .unwrap();
    ...
    metadata_store
        .assign_task("task-1", "test-node-1")
        .await
        .unwrap();
    ...
    assert_eq!(
        ...
        voice_cli::models::TaskState::Completed
    );
...
async fn test_atomic_health_monitoring() {
    ...
    metadata_store
        .update_node_health_atomic("health-test-node", false)
        .await
        .unwrap();
    ...
    // Test recovery
    metadata_store
        .update_node_health_atomic("health-test-node", true)
        .await
        .unwrap();
...
async fn test_cluster_stats_atomic() {
    ...
    for i in 1..=5 {
        ...
        if i <= 2 {
            metadata_store
                .assign_task(&format!("task-{}", i), "node-1")
                .await
                .unwrap();
        }
    }
