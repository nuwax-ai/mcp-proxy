...
use voice_cli::{
    ...
    models::{ClusterNode, LoadBalancerConfig, MetadataStore, NodeRole, NodeStatus},
};
...
async fn test_load_balancer_startup_and_health() {
    ...
    let lb_handle = tokio::spawn(async move { load_balancer.start().await });
    ...
    let health_response = client.get("http://127.0.0.1:8090/health").send().await;
...
async fn test_load_balancer_failover_scenarios() {
    ...
    assert!(
        failover_response.status() == StatusCode::OK
...
async fn test_load_balancer_circuit_breaker() {
    ...
    assert!(
        ...
        "Expected error status code on recovery, got: {}",
...
async fn start_real_voice_cli_server(
    bind_addr: &str,
    healthy: bool,
