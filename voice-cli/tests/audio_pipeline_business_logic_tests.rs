...
use voice_cli::{
    error::VoiceCliError,
    models::Config,
    services::{AudioFormatDetector, AudioProcessor, TranscriptionService},
};
...
async fn test_audio_processor_business_logic() {
    ...
    let result = processor
        .process_audio(wav_data.clone(), Some("test.wav"))
        .await;
    assert!(result.is_ok());
...
async fn test_audio_format_business_logic() {
    ...
    assert_eq!(
        AudioFormat::from_filename("test.unknown"),
        AudioFormat::Unknown
    );
...
async fn test_audio_validation_business_logic() {
    ...
    let validation_result =
        voice_cli::server::handlers::validate_audio_file(&small_audio, "test.wav", max_file_size);
    assert!(validation_result.is_ok());
    ...
    let mp3_validation =
        voice_cli::server::handlers::validate_audio_file(&valid_mp3, "test.mp3", max_file_size);
...
async fn test_audio_processing_error_handling() {
    ...
    let invalid_processing = processor
        .process_audio(invalid_format, Some("test.wav"))
        .await;
...
async fn test_business_logic_integration() {
    ...
    let process_result = processor
        .process_audio(test_audio.clone(), Some("test.wav"))
        .await
        .unwrap();
    assert_eq!(process_result.original_format, Some("wav".to_string()));

    // Step 2: Process audio
    let processed_audio = processor
        .process_audio(test_audio.clone(), Some("test.wav"))
        .await
        .unwrap();
