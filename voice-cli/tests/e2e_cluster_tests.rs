...
use voice_cli::{
    models::{Config, cluster::{TaskMetadata, TaskState}, MetadataStore},
    cluster::{
        task_scheduler::SimpleTaskScheduler,
        transcription_worker::SimpleTranscriptionWorker,
    },
...
async fn test_e2e_metadata_store_operations() {
    ...
    assert!(store_result.is_ok(), "Failed to store task metadata: {:?}", store_result);
...
async fn test_e2e_task_state_transitions() {
    ...
    metadata_store.assign_task(&task.task_id, "worker-node-001").await.unwrap();
    
    // Test processing
    task.mark_processing();
    assert_eq!(task.state, TaskState::Processing);
    metadata_store.start_task_processing(&task.task_id).await.unwrap();
    ...
    metadata_store.complete_task(&task.task_id, 2.5).await.unwrap();
    
    // Verify final state in database
    let stored_task = metadata_store.get_task("state-test-001").await.unwrap().unwrap();
...
async fn test_e2e_task_scheduler_integration() {
    ...
    for task in &tasks {
        ...
        assert!(store_result.is_ok(), "Failed to store task: {:?}", store_result);
    }
...
fn create_test_config(temp_dir: &TempDir) -> Arc<Config> {
    ...
    config.cluster.metadata_db_path = temp_dir.path().join("cluster_data").to_string_lossy().to_string();
