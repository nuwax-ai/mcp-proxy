...
mod cluster_lifecycle_integration_tests {
    ...
    use voice_cli::{
        cluster::{
            task_scheduler::{SchedulerConfig, SchedulerEvent, SimpleTaskScheduler},
            ClusterState,
        },
        models::{
            AudioProcessingConfig, ClusterConfig, ClusterNode, Config, DaemonConfig, LoggingConfig,
            MetadataStore, NodeRole, NodeStatus, ServerConfig, TaskMetadata, TaskState,
    ...
    fn create_test_config(
        ...
        Config {
            ...
            load_balancer: voice_cli::models::LoadBalancerConfig {
                ...
                pid_file: temp_dir
                    .path()
                    .join("load_balancer.pid")
                    .to_string_lossy()
                    .to_string(),
                log_file: temp_dir
                    .path()
                    .join("logs/load_balancer.log")
                    .to_string_lossy()
                    .to_string(),
            },
        }
    ...
    async fn test_node_joining_lifecycle() {
        ...
        let joining_node = create_test_node(
            "new-node",
            NodeRole::Follower,
            NodeStatus::Joining,
            50060,
        ...
        cluster_state
            .update_node_status("new-node", NodeStatus::Healthy)
            .unwrap();
    ...
    async fn test_failure_scenarios_and_recovery() {
        ...
        cluster_state
            .update_node_status("worker-1", NodeStatus::Unhealthy)
            .unwrap();
        ...
        let replacement_node = create_test_node(
            "worker-3",
            NodeRole::Follower,
            NodeStatus::Healthy,
            50083,
        ...
        let new_task =
            cluster_state.create_task("client-4".to_string(), "audio4.wav".to_string(), None, None);
