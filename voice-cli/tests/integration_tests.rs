...
async fn test_task_cancellation() {
    ...
    assert!(cancel_response.status() == StatusCode::OK || cancel_response.status() == StatusCode::BAD_REQUEST);
}
...
async fn test_sync_transcription_still_works() {
    ...
    assert!(response.status() == StatusCode::OK || response.status().is_client_error() || response.status().is_server_error());
}
...
async fn test_task_management_disabled() {
    ...
    assert!(error_response["error"].as_str().unwrap().contains("not enabled"));
}