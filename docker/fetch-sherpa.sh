...
#
# 用法:
#   bash docker/fetch-sherpa.sh           # 默认 amd64
#   bash docker/fetch-sherpa.sh amd64
#   bash docker/fetch-sherpa.sh arm64
#   bash docker/fetch-sherpa.sh all       # 两个架构都下（双架构构建用）
#
# 可用环境变量覆盖:
...
archive_name() {
    case "$1" in
        ...
        *) echo "❌ 未知架构: $1（支持 amd64 / arm64 / all）" >&2; exit 1 ;;
    esac
}
